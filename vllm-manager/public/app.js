...
const LEGACY_THEME_STORAGE_KEY = "themeMode";
...
const state = {
  ...
  hfAuth: { connected: false, username: null },
  pendingHfDownload: null,
...
const profileRenderer = window.LocalAiProfileRenderer.create({
  ...
  metrics(profile) {
    ...
    return [
      String(cfg.engine || "vllm").toUpperCase(),
...
const runtimeStatusRenderer = window.LocalAiRuntimeStatusRenderer.create({
  ...
  options: {
    ...
    noRunningText: "当前没有运行中的本地模型。启动模型后，这里会显示服务名、API 地址和卸载按钮。",
    loadingTitle: "模型运行时容器正在运行",
...
const statsSummaryRenderer = window.LocalAiStatsSummaryRenderer.create({
  ...
  formatContextUsage,
  options: {
    totalTokensLabel: "历史累计 tokens",
    requestLabel: "历史累计请求",
    liveTokensLabel: "本次运行 tokens",
    liveRequestsLabel: "本次运行请求",
    heroFirstMetric: true,
    includeRuntimeModels: true,
    includePrefixCache: true,
    includeLiveUsage: true,
    preferLiveRuntimeMetrics: true,
    speculativeLabels: {
      dspark: "DSpark 接受率",
      nextn: "NEXTN/MTP 接受率",
      mtp: "MTP 接受率",
    ...
    runtimeConfigLabel: "运行配置",
    runtimeThinkingToggleLabel: "思考严格可切换",
    runtimeUnitScaleLabel: "scale=1 灰度",
  },
});
...
function applyLanguageMode() {
  ...
  // The label text is also the accessible name and the tooltip: below 820px the
  // span is hidden and the icon is all that remains, so title/aria-label have to
  // track the same copy rather than the phrase-map translator.
...
// Keeps the visible label, the tooltip, and the accessible name in sync for one
// nav entry. Needed because the label collapses to an icon on narrow screens.
...
function scheduleLanguageTranslation() {
  ...
  const flush = () => {
    ...
    roots.filter((root) => root?.isConnected).forEach((root) => translatePageText(root));
  };
...
function translatePageText(root = document.body) {
  ...
  if (root === document.body) translateDefaultFormValues(lang);
}
    ...
    stages: [
      {
        id: "model",
        label: "选择模型",
        description: "模型、服务名和方案",
        help: "先确认要启动的 checkpoint。套用方案是显式动作，不会因选择模型自动覆盖上下文和并发。",
        selectors: ["#serviceProfileStrip", "#startModel", "#servedName", "#servicePort", "#instanceMode", "#instanceId", "#instanceModeHint"],
      },
      {
        id: "resources",
        label: "资源配置",
        description: "上下文、显存和 GPU",
        help: "上下文与并发保持你的输入；只有点击自动调优或套用方案才会改写。显存按替换当前容器后的容量评估。",
        selectors: [".context-section", "#launchDtype", "#launchQuantization", "#loadFormat", "#ggufSection", ".gpu-section", "#multiGpuMode", "[name='distributedExecutorBackend']", ".parallel-size-grid", "#parallelHintNote", "[name='enableExpertParallel']"],
      },
      {
        id: "capabilities",
        label: "模型能力",
        description: "思考、工具和压缩",
        help: "Reasoning parser 只负责拆分思考内容，不决定模型是否思考；是否思考由客户端请求参数控制。",
        selectors: ["#clientPreset", "#reasoningParser", "#reasoningNote", ".tool-section", ".compression-section", "[name='trustRemoteCode']"],
      },
      {
        id: "access",
        label: "访问与确认",
        ...
        selectors: ["#networkAccess", "#vllmApiKey", "#networkNote", "#startForm button[type='submit']"],
      },
    ],
    renderReview: renderVllmLaunchReview,
    onRefresh: updateLaunchActionState,
  });
}

function renderVllmLaunchReview() {
  ...
  const speculative = describeEffectiveSpeculativeMode();
  ...
  const runtimeEngine = $("#runtimeEngine")?.value === "sglang" ? "SGLang" : "vLLM";
  ...
  return `
    ...
    <div class="launch-review-grid">
      ...
      <div><span>引擎</span><strong>${runtimeEngine}</strong></div>
      <div><span>上下文 / 并发</span><strong>${fmtTokens(Number($("#maxModelLen")?.value || 0))} / ${fmtNumber(Number($("#maxNumSeqs")?.value || 0))}</strong></div>
      <div><span>GPU</span><strong>${escapeHtml(selectedGpus.map((gpu) => gpu.id).join(", ") || "未检测")}</strong></div>
      <div><span>量化 / KV</span><strong>${escapeHtml($("#launchQuantization")?.value || "自动")} / ${escapeHtml($("#kvCacheDtype")?.value || "auto")}</strong></div>
      <div><span>推测解码</span><strong>${escapeHtml(speculative)}</strong></div>
      <div><span>实例 / 端口</span><strong>${escapeHtml(instanceMode === "parallel" ? `并行 ${instanceId}` : "替换主实例")} / ${escapeHtml($("#servicePort")?.value || "-")}</strong></div>
      <div><span>访问</span><strong>${network}${$("#vllmApiKey")?.value.trim() ? " / 已设置 Key" : " / 无 Key"}</strong></div>
    </div>
    <p class="launch-review-note">${instanceMode === "parallel" ? "确认后保留所有现有实例并新建独立容器；请确保端口和 GPU 显存可用。表单值不会被自动改写。" : running ? `确认后只替换主运行容器并切换到 ${runtimeEngine}；失败会自动恢复原容器。` : "确认后创建主实例启动任务；所有参数以本页最终值为准。"}</p>
  `;
}

function effectiveSpeculativeModeForEstimate() {
  const selected = $("#speculativeMode")?.value || "off";
  if (selected !== "auto") return selected;
  const capability = state.modelConfig?.data?.speculative;
  if (capability) return capability.enabled ? capability.mode || "mtp" : "off";
  const model = $("#startModel")?.value || "";
  const quant = `${$("#launchQuantization")?.value || ""} ${model}`.toLowerCase();
  if (/mtp/i.test(model) && !/nvfp4|modelopt|fp4/.test(quant)) return "mtp";
  return "off";
}

function describeEffectiveSpeculativeMode() {
  const selected = $("#speculativeMode")?.value || "off";
  const effective = effectiveSpeculativeModeForEstimate();
  if (selected === "auto") return effective === "off" ? "自动 / 当前模型关闭" : `自动 / ${effective.toUpperCase()}`;
  if (selected === "off") return "关闭";
  if (selected === "dspark") return `DSPARK / block ${Number($("#dsparkBlockSize")?.value || 7)}`;
  return `${selected.toUpperCase()} / ${Number($("#numSpeculativeTokens")?.value || 1)} tokens`;
}

function launchTransitionInfo() {
...
function updateLaunchActionState() {
  ...
  const { running, same } = launchTransitionInfo();
  const parallel = $("#instanceMode")?.value === "parallel";
  const engine = $("#runtimeEngine")?.value === "sglang" ? "SGLang" : "vLLM";
  const label = parallel ? "新增并行实例" : running ? (same ? "重启并应用参数" : "切换并启动") : `启动 ${engine}`;
...
async function confirmVllmLaunch(payload) {
  const { running, same, name } = launchTransitionInfo();
  ...
  return uiArchitecture.confirmLaunch({
    ...
    body: `${renderVllmLaunchReview()}${warning}`,
    confirmLabel: parallel ? "创建并行实例" : running ? (same ? "确认重启" : `切换到 ${name}`) : "仍然启动",
  });
}

function updateLaunchOptionStates() {
  const speculativeMode = $("#speculativeMode")?.value || "off";
  const runtimeEngine = $("#runtimeEngine")?.value || "vllm";
  const speculativeTokens = $("#numSpeculativeTokens");
  if (speculativeMode === "dspark" && runtimeEngine !== "sglang") $("#runtimeEngine").value = "sglang";
  if (speculativeTokens) speculativeTokens.disabled = ["auto", "off", "dspark"].includes(speculativeMode);
  const dspark = speculativeMode === "dspark";
  if ($("#draftModelField")) $("#draftModelField").hidden = !dspark;
  if ($("#dsparkBlockSizeField")) $("#dsparkBlockSizeField").hidden = !dspark;
  // N-gram prompt-lookup only pays off with several draft tokens. The form
  // default is 1 (the MTP default), so when the user switches to N-gram and has
  // not raised it, bump to a useful value instead of launching a no-op probe.
  if (speculativeMode === "ngram" && speculativeTokens && Number(speculativeTokens.value || 1) <= 1) {
    speculativeTokens.value = 3;
  }
  ...
  if (speculativeNote) {
    const capability = state.modelConfig?.data?.speculative;
    if (speculativeMode === "dspark") {
      speculativeNote.textContent = "DSpark 使用外部草稿模型；管理器将切到 SGLang，并校验 DSparkDraftModel、block size、FlashInfer 与 WSL feature transport。";
    } else if (speculativeMode === "off" && capability?.nativeMtp) {
      speculativeNote.textContent = `权重中检出 ${capability.nativeMtpTensorCount || "已打包"} 个 MTP 张量，但当前关闭推测解码。`;
    } else if (speculativeMode === "auto" && capability?.nativeMtp && !capability.enabled) {
      speculativeNote.textContent = `该模型声明原生 MTP（${capability.nativeMtpLayers || 1} 层）；自动模式因当前量化格式保持关闭，可手动选 MTP 后观察接受率。`;
    } else if (speculativeMode === "auto" && capability?.nativeMtp && capability.enabled) {
      speculativeNote.textContent = `该模型声明原生 MTP（${capability.nativeMtpLayers || 1} 层），自动模式将启用 ${String(capability.mode || "mtp").toUpperCase()}。`;
    } else {
      speculativeNote.textContent = speculativeMode === "auto"
        ? `${describeEffectiveSpeculativeMode()}，token 数由模型能力决定。`
        : speculativeMode === "off"
          ? "已关闭，不预留推测解码显存。"
          : `显式启用 ${speculativeMode.toUpperCase()}，请用接受率和输出吞吐验证是否提速。`;
    }
  }

  const autoTools = $("#enableAutoToolChoice")?.checked;
  const toolParser = $("#toolCallParser");
  if (toolParser) toolParser.disabled = !autoTools;

  const gpuCount = getVisibleGpus().length;
  const multiGpuMode = $("#multiGpuMode");
  if (multiGpuMode) {
    Array.from(multiGpuMode.options).forEach((option) => {
      if (option.value !== "single") option.disabled = gpuCount > 0 && gpuCount < 2;
    });
    if (gpuCount === 1 && multiGpuMode.value !== "single") {
      multiGpuMode.value = "single";
      $("#tensorParallelSize").value = 1;
      $("#pipelineParallelSize").value = 1;
      $("#dataParallelSize").value = 1;
    }
  }
...
function groupContextControls() {
  ...
  const offload = createControlDisclosure("显存 offload 与多模态预留", "CPU offload 是每卡权重回退，KV offload 是总 KV 缓冲；能救容量但会拖慢。", ["#cpuOffloadGb", "#kvOffloadingSize", "#mmProcessorCacheGb", "#disablePrefixCaching", "#languageModelOnly"]);
  const speculative = createControlGroup("推测解码", "MTP 使用主模型内置张量；DSpark 使用外部草稿模型并切换到 SGLang。", ["#speculativeMode", "#numSpeculativeTokens", "#draftModelField", "#dsparkBlockSizeField", "#runtimeImage"]);
  grid.replaceChildren(basic, kv, offload, speculative);
}
...
const connectionStatus = window.ConnectionStatus.create({
  ...
  title: () => t("connection.lostTitle"),
  onDisconnect: (message) => notify(t("connection.lostTitle"), message || t("connection.lostToast"), "error"),
  onReconnect: () => {
    notify(t("connection.restoredTitle"), t("connection.restoredToast"), "success");
...
function bindEvents() {
  ...
  $("#hfAuthLoginBtn")?.addEventListener("click", startHfLogin);
  $("#hfAuthRefreshBtn")?.addEventListener("click", () => refreshHfAuthStatus().catch(() => {}));
  $("#hfAccessRecheckBtn")?.addEventListener("click", recheckHfAccessAndContinue);
  ...
  ["#startModel", "#runtimeEngine", "#maxModelLen", "#maxNumSeqs", "#gpuMemoryUtilization", "#cpuOffloadGb", "#kvOffloadingSize", "#mmProcessorCacheGb", "#launchDtype", "#launchQuantization", "#loadFormat", "#kvCacheDtype", "#tensorParallelSize", "#pipelineParallelSize", "#dataParallelSize", "#languageModelOnly", "#speculativeMode", "#numSpeculativeTokens", "#dsparkBlockSize"].forEach((selector) => {
    const node = $(selector);
    if (!node) return;
  ...
  // Quantization changes the model's capability profile (MTP auto on/off for
  // NVFP4 vs FP16), so re-evaluate it instead of keeping a stale cache entry.
  $("#launchQuantization").addEventListener("change", scheduleModelConfigFetch);
  $("#loadFormat").addEventListener("change", updateGgufModeState);
  $("#speculativeMode").addEventListener("change", updateLaunchOptionStates);
  $("#runtimeEngine")?.addEventListener("change", () => {
    if ($("#runtimeEngine").value === "vllm" && $("#speculativeMode").value === "dspark") {
      $("#speculativeMode").value = "mtp";
      $("#numSpeculativeTokens").value = 3;
    }
    updateLaunchOptionStates();
  });
  $("#enableAutoToolChoice").addEventListener("change", updateLaunchOptionStates);
...
let hfAuthPollGeneration = 0;

function renderHfAuthStatus(status = state.hfAuth) {
  state.hfAuth = status || { connected: false, username: null };
  const card = $("#hfAuthCard");
  const text = $("#hfAuthStatus");
  const loginButton = $("#hfAuthLoginBtn");
  if (!card || !text) return;
  if (state.hfAuth.connected) {
    card.dataset.state = "ok";
    text.textContent = `已连接${state.hfAuth.username ? `：${state.hfAuth.username}` : " Hugging Face"}`;
    if (loginButton) loginButton.textContent = "重新登录";
  } else {
    card.dataset.state = state.hfAuth.checking ? "loading" : "warn";
    text.textContent = state.hfAuth.checking ? "正在检查本机登录状态..." : "本机尚未连接 Hugging Face 账号";
    if (loginButton) loginButton.textContent = "登录 Hugging Face";
  }
}

async function refreshHfAuthStatus({ silent = false } = {}) {
  if (!silent) renderHfAuthStatus({ ...state.hfAuth, checking: true });
  try {
    const status = await api("/api/huggingface/auth/status");
    renderHfAuthStatus(status);
    return status;
  } catch (error) {
    renderHfAuthStatus({ connected: false, username: null, reason: error.message });
    if (!silent) notify("HF 账号状态检查失败", error.message, "error");
    throw error;
  }
}

async function startHfLogin() {
  const button = $("#hfAuthLoginBtn");
  const clearBusy = setButtonBusy(button, "等待登录...");
  try {
    await api("/api/huggingface/auth/login", { method: "POST", body: "{}" });
    notify("已打开 HF 登录窗口", "请在新窗口粘贴 read 或 fine-grained read Token；页面会自动检测登录结果。", "info");
    beginHfAuthPolling();
  } catch (error) {
    reportActionError("无法打开 HF 登录窗口", error);
  } finally {
    clearBusy();
  }
}

async function beginHfAuthPolling() {
  const generation = ++hfAuthPollGeneration;
  for (let attempt = 0; attempt < 90 && generation === hfAuthPollGeneration; attempt += 1) {
    await new Promise((resolve) => setTimeout(resolve, 2000));
    const status = await refreshHfAuthStatus({ silent: true }).catch(() => null);
    if (!status?.connected) continue;
    notify("Hugging Face 登录成功", status.username || "本机凭据已生效", "success");
    if (state.pendingHfDownload) await recheckHfAccessAndContinue();
    return;
  }
}

function renderHfGatedAccess(estimate = {}) {
  const panel = $("#hfGatedAccess");
  const text = $("#hfGatedAccessText");
  const link = $("#hfAuthorizationLink");
  ...
  link.href = estimate.authorizationUrl || `https://huggingface.co/${encodeURI(estimate.model || $("#downloadModel")?.value.trim() || "")}`;
  text.textContent = state.hfAuth.connected
    ? `账号${state.hfAuth.username ? ` ${state.hfAuth.username}` : ""}已连接；请确认该账号已在官方模型页获得访问权。`
    : "先登录 Hugging Face，再在官方模型页确认访问条款。";
}

async function recheckHfAccessAndContinue() {
  const model = String(state.pendingHfDownload?.model || $("#downloadModel")?.value || "").trim();
  const precision = String(state.pendingHfDownload?.precision || $("#downloadPrecision")?.value || "").trim();
  if (!model) return;
  const button = $("#hfAccessRecheckBtn");
  const clearBusy = setButtonBusy(button, "正在复检...");
  try {
    const params = new URLSearchParams({ model, precision });
    const access = await api(`/api/huggingface/access?${params}`);
    const panel = $("#hfGatedAccess");
    const text = $("#hfGatedAccessText");
    if (!access.granted) {
      if (panel) panel.dataset.state = "warn";
      if (text) text.textContent = access.status === "login_required"
        ? "本机仍未登录 Hugging Face；请先完成登录窗口。"
        : "当前账号仍未取得仓库文件访问权；请在官方模型页确认授权后再复检。";
      notify("HF 授权尚未通过", text?.textContent || access.status, "info");
      return;
    }
    if (panel) panel.dataset.state = "ok";
    if (text) text.textContent = `账号${access.username ? ` ${access.username}` : ""}和模型授权均已通过。`;
    notify("HF 授权已通过", state.pendingHfDownload ? "正在继续刚才的下载请求。" : "现在可以开始下载。", "success");
    if (state.pendingHfDownload) {
      const payload = state.pendingHfDownload;
      state.pendingHfDownload = null;
      await submitDownloadPayload(payload);
    }
  } catch (error) {
    reportActionError("HF 授权复检失败", error);
  } finally {
    clearBusy();
  }
}
...
async function requestDownloadEstimate() {
  ...
  if (!model || !model.includes("/")) {
    box.hidden = true;
    renderHfGatedAccess({ gated: false });
    updateDownloadPlanPreview();
    return;
  }
  if (source !== "huggingface") {
...
function renderModelPicker() {
  ...
  window.modelPickerRenderer?.render({
    ...
    dialogTitle: english ? "Choose a vLLM model" : "选择 vLLM 模型",
    dialogDescription: english ? "Filter by source, format and post-switch GPU capacity." : "按来源、格式和切换后的显存容量筛选。",
...
function buildModelPickerItems() {
  ...
  (state.models.local || []).forEach((model) => {
    ...
    add({
      ...
      disabled: model.runnable === false,
      disabledReason: model.verificationIssues?.find((item) => item.severity === "fail")?.detail || "模型完整性校验未通过",
...
function estimateModelFit(item) {
  ...
  const planned = selected.map(normalizeVllmPlanGpu);
  ...
  if (selected.length > 1 && modelGb + headroomGb <= totalFreeGb * 0.82) return { label: `需多卡${capacitySuffix}`, state: "warn" };
...
function getLiveTokensPerSecond() {
  const totals = state.stats?.live?.totals || state.stats?.totals || {};
  const candidates = [
    totals.speed?.recentOutputTokensPerSecond,
...
function getLaunchFormConfig() {
  ...
  ["trustRemoteCode", "enableExpertParallel", "disablePrefixCaching", "languageModelOnly", "enableAutoToolChoice"].forEach((key) => {
...
function applyLaunchProfile(config = {}, options = {}) {
  ...
  if (config.reasoningParser && config.reasoningParser !== "auto") state.reasoningParserTouched = true;
  if (config.toolCallParser && config.toolCallParser !== "auto") state.toolCallParserTouched = true;
  ...
  set("#speculativeMode", config.speculativeMode === "qwen3_next_mtp" ? "mtp" : config.speculativeMode);
  set("#numSpeculativeTokens", config.numSpeculativeTokens);
  set("#runtimeImage", config.runtimeImage);
  set("#draftModel", config.draftModel);
  set("#dsparkBlockSize", config.dsparkBlockSize);
  if (Object.hasOwn(config, "runtimeImage") && $("#runtimeImage")) $("#runtimeImage").value = String(config.runtimeImage || "");
  if (Object.hasOwn(config, "draftModel") && $("#draftModel")) $("#draftModel").value = String(config.draftModel || "");
  const checks = {
    trustRemoteCode: "#startForm [name='trustRemoteCode']",
    enableExpertParallel: "#startForm [name='enableExpertParallel']",
    disablePrefixCaching: "#disablePrefixCaching",
  ...
  if (config.enablePrefixCaching === true && $("#disablePrefixCaching")) $("#disablePrefixCaching").checked = false;
...
function renderRuntimeConfigBanner(config, modelId) {
  ...
  banner.innerHTML = `<strong>已载入当前运行参数</strong><span>${escapeHtml(modelId)} · ${fmtTokens(Number(config.maxModelLen || 0))} 上下文 · ${fmtNumber(Number(config.maxNumSeqs || 0))} 并发。只有套用方案或自动调优会改写这些值。</span>`;
}
  ...
  try {
    const result = await api("/api/tools/benchmark", {
      ...
      body: JSON.stringify({
        ...
        concurrency: Number($("#benchmarkConcurrency").value || 1),
        warmupRequests: Number($("#benchmarkWarmupRequests").value || 0),
        stream: Boolean($("#benchmarkStream")?.checked),
...
function normalizeVllmPlanGpu(gpu) {
  const utilization = $("#gpuMemoryUtilization")?.value || 0.9;
...
function buildVllmMemoryEstimatePayload(estimate) {
  return {
    ...
    speculativeMode: effectiveSpeculativeModeForEstimate(),
    numSpeculativeTokens: Number($("#numSpeculativeTokens")?.value || 1),
  };
}
...
function mergeVllmServerMemoryEstimate(estimate, result) {
  ...
  const selectedGpus = Array.isArray(plan.selectedGpus) && plan.selectedGpus.length
    ? plan.selectedGpus.map((gpu) => {
        const local = estimate.selectedGpus.find((item) => String(item.id) === String(gpu.id)) || {};
        return { ...gpu, currentFreeGb: local.currentFreeGb, capacityMode: local.capacityMode };
      })
...
// Cache key combines model + chosen quantization, because the quantization
// dropdown changes the model's capability profile (e.g. NVFP4 disables auto
// MTP for Qwen3.5/3.6) and must invalidate the cached config.
function modelConfigCacheKey() {
  const model = $("#startModel")?.value.trim() || "";
  const quantization = String($("#launchQuantization")?.value || "").trim();
  return quantization ? `${model}|${quantization}` : model;
}
...
async function fetchModelConfigForLaunch() {
  ...
  // Quantization is part of the capability decision (e.g. NVFP4 forces auto-MTP
  // off for Qwen3.5/3.6), so the cache key must include it: changing the
  // dropdown has to re-fetch capabilities instead of reusing a stale result.
  const quantization = String($("#launchQuantization")?.value || "").trim();
  const cacheKey = modelConfigCacheKey();
  if (state.modelConfig.key === cacheKey && state.modelConfig.data) return;
  state.modelConfig = { key: cacheKey, data: null, loading: true };
  try {
    const source = looksLikePathInput(model) ? "local" : "huggingface";
    const params = { model, source };
    if (quantization) params.quantization = quantization;
    const data = await api(`/api/model/config?${new URLSearchParams(params)}`);
    // 期间用户可能又改了模型名/量化，丢弃过期结果
    if ($("#startModel")?.value.trim() !== model || String($("#launchQuantization")?.value || "").trim() !== quantization) return;
    state.modelConfig = { key: cacheKey, data, loading: false };
  } catch (error) {
    if ($("#startModel")?.value.trim() !== model || String($("#launchQuantization")?.value || "").trim() !== quantization) return;
    state.modelConfig = { key: cacheKey, data: { found: false, reason: error.message }, loading: false };
  }
  updateMemoryEstimate();
  updateLaunchOptionStates();
}
...
function getLaunchModelConfig() {
  if (state.modelConfig.key === modelConfigCacheKey() && state.modelConfig.data?.found) return state.modelConfig.data;
  return null;
}
...
function estimateMemoryUsage(contextOverride) {
  ...
  const rawCfg = state.modelConfig.key === modelConfigCacheKey() ? state.modelConfig.data : null;
  const configGated = Boolean(rawCfg && rawCfg.gated && !rawCfg.hasToken);
  // 有 config.json 时用真实层数/KV 头数/headDim，估算精度远高于按模型名猜；
  // 混合模型（linear/mamba + full_attention）只用注意力层数算 KV
  const attentionLayers = Number(cfg?.numAttentionLayers) || cfg?.numHiddenLayers;
  const arch = (cfg && cfg.numHiddenLayers && cfg.numKeyValueHeads && cfg.headDim)
    ? {
        layers: cfg.numHiddenLayers,
        kvLayers: attentionLayers,
        kvHeads: cfg.numKeyValueHeads,
        headDim: cfg.headDim,
        weightMultiplier: 1,
        label: attentionLayers && attentionLayers < cfg.numHiddenLayers
          ? `${cfg.numHiddenLayers} 层（注意力 ${attentionLayers}）· KV heads ${cfg.numKeyValueHeads} · headDim ${cfg.headDim}（来自 config）`
          : `${cfg.numHiddenLayers} 层 · KV heads ${cfg.numKeyValueHeads} · headDim ${cfg.headDim}（来自 config）`,
        fromConfig: true,
      }
  ...
  return {
    ...
    configGated,
    configLoading: state.modelConfig.loading && state.modelConfig.key === modelConfigCacheKey(),
...
function renderGpuMemoryBars(estimate) {
  ...
  return estimate.selectedGpus.map((gpu) => {
    ...
    const currentFreeGb = Number(gpu.currentFreeGb ?? freeGb);
    const capacityText = gpu.capacityMode === "replacement"
      ? `换模后预算 ${formatGbNumber(usableGb)} GB · 当前空闲 ${formatGbNumber(currentFreeGb)} GB`
      : `可用 ${formatGbNumber(usableGb)} GB · 当前空闲 ${formatGbNumber(currentFreeGb)} GB`;
    ...
    return `
      <div class="gpu-bar">
        <div class="gpu-bar-head">
          ...
          <span>${formatGbNumber(estimate.perGpuGb)} GB / ${capacityText}</span>
        </div>
...
function renderMemoryEstimateNote(estimate) {
  ...
  const replacementBudget = estimate.selectedGpus.some((gpu) => gpu.capacityMode === "replacement");
  const headroomLine = estimate.selectedGpus.length
    ? `<span class="memory-headroom">每卡约用 ${formatGbNumber(estimate.perGpuGb)} GB / ${replacementBudget ? "替换当前容器后可用" : "当前可用"} ${formatGbNumber(estimate.minUsableGb)} GB，余量 ${formatGbNumber(Math.max(0, estimate.minUsableGb - estimate.perGpuGb))} GB。${replacementBudget ? "当前模型占用会在新模型启动前释放。" : ""} 估算可信度：${escapeHtml(confidenceText)}。</span>`
    : "";
...
function estimateArchitecture(paramsB, model) {
  ...
  if ((lower.includes("qwen3.6") || lower.includes("qwen3.8") || lower.includes("qwen3_5") || lower.includes("qwen3-5")) && paramsB >= 20 && paramsB <= 35) {
    return {
      layers: 64,
      kvLayers: 16,
      kvHeads: 4,
...
function updateReasoningNote() {
  ...
  $("#reasoningNote").innerHTML = `
    ...
    <span>${escapeHtml(parserText)} Parser 只拆分输出字段，不控制模型是否思考；是否思考由客户端请求的 chat_template_kwargs 等参数决定。 ${escapeHtml(presetText)}</span>
  `;
}
...
function inferReasoningParser(model, preset) {
  ...
  if (text.includes("qwen3") || text.includes("qwen-3") || text.includes("qwen3.5") || text.includes("qwen3.6") || text.includes("qwen3.8")) return "qwen3";
...
function inferToolCallParser(model, preset) {
  ...
  if (text.includes("diffusiongemma") || text.includes("diffusion_gemma") || text.includes("gemma4") || text.includes("gemma-4")) return "gemma4";
  if (text.includes("qwen3.6") || text.includes("qwen3.8") || text.includes("qwen3-") || text.includes("qwen/qwen3") || text.includes("qwen3_coder") || text.includes("qwen3-coder")) return "qwen3_coder";
...
function renderStats() {
  ...
  window.statsUiRenderer.renderHistoryTrends(stats, {
    ...
    labels: effectiveLanguage() === "en-US" ? {} : {
      ...
      gpuPower: "GPU 功耗",
      gpuFan: "GPU 风扇",
      mtpAcceptance: stats.live?.totals?.speculative?.mode === "dspark"
        ? "DSpark 接受率"
        : stats.live?.totals?.speculative?.mode === "nextn"
          ? "NEXTN/MTP 接受率"
          : "MTP 接受率",
    },
  });
    ...
    return `
      <div class="gpu-detail-card">
        ...
        <div class="gpu-detail-telemetry">
          <span class="gpu-detail-temp ${tempState}">温度 ${Number.isFinite(gpu.temp) ? `${gpu.temp}°C` : "-"}</span>
          <span>功耗 ${gpu.powerWatts !== null && gpu.powerWatts !== undefined && gpu.powerWatts !== "" && Number.isFinite(Number(gpu.powerWatts)) ? `${Number(gpu.powerWatts).toFixed(0)} W` : "-"}${gpu.powerLimitWatts !== null && gpu.powerLimitWatts !== undefined && gpu.powerLimitWatts !== "" && Number.isFinite(Number(gpu.powerLimitWatts)) ? ` / ${Number(gpu.powerLimitWatts).toFixed(0)} W` : ""}</span>
          <span>风扇 ${gpu.fanPercent !== null && gpu.fanPercent !== undefined && gpu.fanPercent !== "" && Number.isFinite(Number(gpu.fanPercent)) ? `${Number(gpu.fanPercent).toFixed(0)}%` : "-"}</span>
        </div>
      </div>
    `;
  }).join("");
...
function renderStatsModels(stats) {
  window.statsListRenderer.renderModels(stats, {
    ...
    labels: {
      ...
      running: "运行中",
      historical: "历史累计",
      currentRunTokens: "本次 Token",
      currentRunRequests: "本次请求",
      cumulative: "累计",
    },
  });
}
...
function renderStatsCosts(stats) {
  window.statsUiRenderer.renderCosts(stats, {
    root: $("#statsCostTable"),
    managerName: stats?.live?.engine === "sglang" ? "SGLang" : "vLLM",
    escapeHtml,
    fmtMoney,
    labels: {
      empty: "暂无价格折算。",
  ...
  window.LocalAiLocalModelRenderer.render({
    ...
    onDelete: async ({ button, name, size }) => {
      const ok = await uiArchitecture.confirmAction({
        title: `删除本地模型 ${name}？`,
        body: `<p>将从磁盘移除整个目录（${escapeHtml(size || "未知大小")}），无法恢复；正在运行的服务不会自动停止。</p>`,
        confirmLabel: "删除模型",
        danger: true,
      });
      if (!ok) return;
...
function renderRemoteModels() {
  ...
  if (hint) {
    ...
    hint.textContent = english
      ? `Online catalog · returned ${fmtNumber(state.remoteModels.length)}, showing ${fmtNumber(models.length)}. Parameters and file sizes are estimated from public metadata; gated models require a token.`
      : `按「${sortFilter}」排序 · ${taskFilter} · ${sizeFilter} ${featureFilter} · 返回 ${fmtNumber(state.remoteModels.length)} 个，显示 ${fmtNumber(models.length)} 个${unknownHint}。参数和文件大小按公开元数据估算，gated 模型下载前需配置 token。`;
  }
...
function renderBenchmarkProgress(job) {
  ...
  const detail = benchmark.avgTokensPerSecond
    ? `${Number(benchmark.wallTokensPerSecond || benchmark.avgTokensPerSecond).toFixed(2)} tok/s · C${benchmark.concurrency || 1} · TTFT ${fmtMs(benchmark.avgTtftMs || 0)}`
...
async function downloadModel(event) {
  ...
  try {
    ...
    try {
      ...
      if (["hf_login_required", "hf_invalid_token"].includes(error.code)) {
        state.pendingHfDownload = { ...payload };
        delete state.pendingHfDownload.hfToken;
        renderHfGatedAccess({
          gated: true,
          model,
          authorizationUrl: error.authorizationUrl || estimate.authorizationUrl,
        });
        notify("下载已等待 HF 登录", "完成新打开的登录窗口后，平台会自动复检并继续本次下载。", "info");
        await startHfLogin();
        return;
      }
      if (error.code === "hf_approval_required") {
        state.pendingHfDownload = payload;
        renderHfGatedAccess({
          gated: true,
          model,
        ...
        notify("下载已等待模型授权", "请打开官方授权页确认条款，然后点“已完成，复检并继续”。", "info");
        return;
      }
      throw error;
    }
...
async function submitDownloadPayload(payload) {
  await api("/api/download", {
    method: "POST",
    body: JSON.stringify(payload),
  });
  notify("下载任务已创建", payload.model, "success");
...
async function startService(event) {
  event.preventDefault();
  if (!validateVllmLaunchForm(event.currentTarget)) return;
  const payload = buildVllmLaunchPayload(event.currentTarget);
  if (!await confirmVllmLaunch(payload)) return;
...
function validateVllmLaunchForm(form) {
  ...
  const nonNegative = (selector, label, max = 1_048_576) => {
    const control = $(selector);
    if (!control) return true;
    const value = Number(control.value);
    return Number.isFinite(value) && value >= 0 && value <= max
      ? true
      : fail(control, `${label}必须是 0–${max} 的数字。`);
  };
  ...
  if (!integer("#maxModelLen", "上下文长度", 1, 4_194_304)) return false;
  if (!integer("#maxNumSeqs", "最大并发序列", 1, 1024)) return false;
  if (!integer("#maxNumBatchedTokens", "调度 token 上限", 0, 4_194_304)) return false;
  if (!integer("#tensorParallelSize", "张量并行", 1, 1024)) return false;
  if (!integer("#pipelineParallelSize", "流水并行", 1, 1024)) return false;
  if (!integer("#dataParallelSize", "数据并行", 1, 1024)) return false;
  if (!integer("#numSpeculativeTokens", "推测 token 数", 1, 256)) return false;
  if (!integer("#dsparkBlockSize", "DSpark block size", 1, 32)) return false;
  ...
  if (!nonNegative("#cpuOffloadGb", "CPU offload", 4096)) return false;
  if (!nonNegative("#kvOffloadingSize", "KV offload", 4_194_304)) return false;
  if (!nonNegative("#mmProcessorCacheGb", "多模态缓存", 4096)) return false;
  return true;
}

function buildVllmLaunchPayload(formElement) {
  const form = new FormData(formElement);
  const payload = Object.fromEntries(form.entries());
  payload.gpuDeviceIds = form.getAll("gpuDeviceIds");
  payload.trustRemoteCode = form.get("trustRemoteCode") === "on";
  payload.enableExpertParallel = form.get("enableExpertParallel") === "on";
  payload.disablePrefixCaching = form.get("disablePrefixCaching") === "on";
  ...
  payload.maxNumBatchedTokens = Number(payload.maxNumBatchedTokens || 0);
  ...
  payload.speculativeMode = String(payload.speculativeMode || "off");
  payload.numSpeculativeTokens = Number(payload.numSpeculativeTokens || 1);
  payload.engine = payload.engine === "sglang" ? "sglang" : "vllm";
  payload.runtimeImage = String(payload.runtimeImage || "").trim();
  payload.draftModel = String(payload.draftModel || "").trim();
  payload.dsparkBlockSize = Number(payload.dsparkBlockSize || 7);
  return payload;
}
...
async function handleRunningModelAction(event) {
  ...
  if (action === "unload-model") {
    ...
    const ok = await uiArchitecture.confirmAction({
      title: "卸载运行中的模型？",
      body: `<p>将停止 vLLM 容器（${escapeHtml(modelLabel)}），正在使用模型的客户端会立即断开。模型文件保留在磁盘上，管理器和其他 Docker 服务不受影响。</p>`,
      confirmLabel: "卸载模型",
      danger: true,
    });
    if (!ok) return;
...
async function handleServiceJobAction(event) {
  ...
  try {
    if (action === "start-docker") {
      ...
      notify("已请求启动 Docker", "Docker 引擎初始化可能需要几十秒。", "success");
...
async function stopService(event) {
  const ok = await uiArchitecture.confirmAction({
    title: "停止 vLLM 服务？",
    body: `<p>将停止并移除受管的 vLLM 容器，正在使用模型的客户端会立即断开。模型文件保留在磁盘上，OpenWebUI 等其他 Docker 服务不受影响。</p>`,
    confirmLabel: "停止服务",
    danger: true,
  });
  if (!ok) return;
