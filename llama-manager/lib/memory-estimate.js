...
function buildLlamaMemoryEstimate(input = {}) {
  ...
  const variant = input.variant || input.selectedVariant || input.modelMetadata || null;
  const arch = normalizeMemoryEstimateArch(input.arch || variant || input.modelConfig || input.config);
  const parallelSlots = Math.max(1, Math.floor(memoryEstimateNumber(input.maxNumSeqs ?? input.parallelSlots, 1)));
  // Projectors are optional runtime components. Keep the recommendation text-only
  // unless the caller explicitly selected multimodal mode; merely discovering an
  // mmproj next to the weights must not add its VRAM or enable vision implicitly.
  const textOnlyMode = input.textOnlyMode !== false && input.languageModelOnly !== false;
  ...
  const explicitDraftBytes = memoryEstimateNumber(input.dflash?.fileBytes ?? input.draft?.fileBytes ?? input.draftFileBytes, 0);
  const speculativeReserveGb = explicitDraftBytes > 0
    ? explicitDraftBytes / (1024 ** 3)
    : speculativeMode === "off" ? 0 : Math.min(4, 0.5 + speculativeTokens * 0.25);
  const availableMmprojBytes = memoryEstimateNumber(input.mmproj?.fileBytes ?? input.mmprojFileBytes, 0);
  const explicitMmprojBytes = textOnlyMode ? 0 : availableMmprojBytes;
  const paramsB = memoryEstimateNumber(input.paramsB ?? variant?.paramsB, 0);
  const parameterCount = memoryEstimateNumber(variant?.parameterCount, paramsB * 1e9);
  const variantFileBytes = memoryEstimateNumber(variant?.fileBytes ?? variant?.size, 0);
  const bytesPerParam = memoryEstimateNumber(
    input.bytesPerParam,
    variantFileBytes > 0 && parameterCount > 0 ? variantFileBytes / parameterCount : 0.56,
  );
  const accurateKvBytes = variant ? core.estimateGgufKvBytes(variant, {
    maxModelLen: Math.max(1, memoryEstimateNumber(input.contextTokens ?? input.maxModelLen, 8192)),
    maxNumSeqs: parallelSlots,
    cacheTypeK: input.cacheTypeK || input.kvCacheDtype,
    cacheTypeV: input.cacheTypeV || input.kvCacheDtype,
  }) : null;
  const plan = core.estimateLlamaMemoryPlan({
    ...
    bytesPerParam: Math.max(0.125, bytesPerParam),
    ...
    multimodalReserveGb: Math.max(0, memoryEstimateNumber(
      input.multimodalReserveGb,
      explicitMmprojBytes > 0 ? explicitMmprojBytes / (1024 ** 3) : input.arch?.isMultimodal ? 2 : 0,
    )) + speculativeReserveGb,
    kvCacheGb: accurateKvBytes === null ? null : accurateKvBytes / (1024 ** 3),
  });
  ...
  return {
    ...
    metadata: variant ? {
      ...
      parameterCount: variant.parameterCount || null,
      paramsB: variant.paramsB || paramsB || null,
      fileBytes: variant.fileBytes || variant.size || null,
      layers: variant.layers || null,
      kvHeads: variant.kvHeads || null,
      headDim: variant.headDim || null,
      contextLength: variant.contextLength || null,
      slidingWindow: variant.slidingWindow || null,
      slidingLayers: variant.slidingLayers ?? null,
      globalLayers: variant.globalLayers ?? null,
    } : null,
    selectedVariant: variant,
    components: {
      mmprojBytes: explicitMmprojBytes,
      availableMmprojBytes,
      draftBytes: explicitDraftBytes,
    },
    recommendedConfig: variant ? {
      maxModelLen: variant.contextLength || input.maxModelLen || 8192,
      maxNumSeqs: Number(variant.contextLength || 0) >= 131072 ? 1 : parallelSlots,
      cacheTypeK: input.cacheTypeK || input.kvCacheDtype || "q8_0",
      cacheTypeV: input.cacheTypeV || input.kvCacheDtype || "q8_0",
      gpuLayers: plan.recommendedGpuLayers >= plan.totalLayers ? "all" : plan.recommendedGpuLayers,
      // The estimate follows the user's current text/vision choice, while the
      // model recommendation stays on the safer text-only default. The
      // detected projector remains available for an explicit vision opt-in.
      textOnlyMode: true,
      languageModelOnly: true,
      mmproj: "",
      availableMmproj: input.mmproj?.path || "",
      draftModel: input.dflash?.path || input.draft?.path || "",
      speculativeMode: explicitDraftBytes > 0 ? "draft-dflash" : speculativeMode,
      ...(String(variant.architecture || "").toLowerCase() === "muse-glimmer" ? {
        temperature: 1,
        topP: 0.95,
        topK: 64,
      } : {}),
