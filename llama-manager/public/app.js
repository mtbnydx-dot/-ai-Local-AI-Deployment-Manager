...
const {
  variantsFor: ggufVariantsFor,
  componentsFor: ggufComponentsFor,
  selectedVariant: selectedGgufVariant,
  renderLibrary: renderGgufLibrary,
} = window.LlamaGgufUi;
...
const { EN_TEXT, ZH_TEXT, UI_COPY, UI_TRANSLATION_PATTERNS, UI_PARTIAL_TRANSLATIONS } = window.LlamaI18n;
const { setButtonBusy, notify, reportActionError } = window.LlamaUiUtils.create({ $, escapeHtml, renderIcons: () => renderIcons() });
const sparkline = (values, options = {}) => window.statsUiRenderer.renderSparkline(
  values,
  options.color || "var(--blue)",
  options.label || "趋势图",
);
...
function updateLaunchOptionStates() {
  ...
  if (speculativeTokens) speculativeTokens.disabled = !speculativeMode.startsWith("draft-");
      ...
      : speculativeMode === "off"
        ...
        : speculativeMode === "draft-mtp"
          ...
          : speculativeMode === "draft-dflash"
            ? "显式启用 DFlash draft；主模型和 draft 组件会分开加载。"
...
function effectiveLlamaSpeculativeModeForEstimate() {
  ...
  const selected = getSelectedGgufInfo();
  const recommended = selected?.variant?.recommendations?.speculativeMode;
  if (recommended) return recommended;
  if (selected?.components?.some((component) => component.kind === "dflash")) return "draft-dflash";
  return /(?:^|[-_/.])mtp(?:$|[-_/.])/i.test($("#startModel")?.value || "") ? "draft-mtp" : "off";
}

function getSelectedGgufInfo() {
  const models = [...(state.models?.local || []), ...(state.models?.cached || [])];
  const value = $("#startModel")?.value || "";
  const exact = selectedGgufVariant(models, value);
  if (exact) return mergeSelectedGgufServerDetails(exact);
  const key = normalizePathKey(value);
  const model = models.find((item) => [item.path, item.id, item.label, item.launchModel].some((candidate) => normalizePathKey(candidate) === key));
  if (!model) return null;
  const variants = ggufVariantsFor(model);
  const variant = variants.find((item) => item.selected) || variants[0];
  return variant ? mergeSelectedGgufServerDetails({ model, variant, components: ggufComponentsFor(model) }) : null;
}

function mergeSelectedGgufServerDetails(selected) {
  const server = state.memoryEstimateServer;
  if (!server || normalizePathKey(server.model) !== normalizePathKey($("#startModel")?.value || "")) return selected;
  const variant = { ...selected.variant };
  if (server.selectedVariant && normalizePathKey(server.selectedVariant.launchModel || server.selectedVariant.path) === normalizePathKey(variant.launchModel)) {
    Object.assign(variant, server.selectedVariant, { raw: server.selectedVariant });
  }
  variant.recommendations = server.recommendedConfig || variant.recommendations;
  return {
    ...selected,
    variant,
    components: Array.isArray(server.components) && server.components.length ? normalizeServerEstimateComponents(server.components, selected.components) : selected.components,
  };
}

function normalizeServerEstimateComponents(components, fallback) {
  const normalized = components.map((component) => {
    const rawKind = String(component.kind || component.type || component.role || "component").toLowerCase();
    return {
      kind: rawKind.includes("mm") || rawKind.includes("project") ? "mmproj" : rawKind.includes("draft") || rawKind.includes("dflash") ? "dflash" : rawKind,
      path: component.path || component.launchModel || component.name || "",
      name: component.name || String(component.path || "").replace(/\\/g, "/").split("/").pop(),
      fileBytes: Number(component.fileBytes || component.sizeBytes || component.size || 0),
    };
...
function renderSelectedModelDetails() {
  const root = $("#selectedModelDetails");
  if (!root) return;
  const selected = getSelectedGgufInfo();
  if (!selected) {
    root.innerHTML = '<div class="empty compact">选择本地 GGUF 文件或具体变体后，会在这里显示真实元数据与兼容性；远程仓库在下载后补全。</div>';
    return;
  }
  const { model, variant, components } = selected;
  const compatibility = variant.compatibility || { state: "unknown", label: "待核验", detail: "后端尚未报告兼容性。" };
  const metrics = [
    variant.fileBytes ? ["主权重", fmtBytes(variant.fileBytes)] : null,
    variant.paramsB ? ["参数量", `${variant.paramsB >= 10 ? variant.paramsB.toFixed(0) : variant.paramsB.toFixed(1)}B`] : null,
    variant.architecture ? ["架构", variant.architecture] : null,
    variant.layers ? ["层数", fmtNumber(variant.layers)] : null,
    variant.kvLayers && variant.kvLayers !== variant.layers ? ["KV 层数", fmtNumber(variant.kvLayers)] : null,
    variant.embeddingLength ? ["Embedding", fmtNumber(variant.embeddingLength)] : null,
    variant.attentionHeads ? ["Attention heads", fmtNumber(variant.attentionHeads)] : null,
    variant.kvHeads ? ["KV heads", fmtNumber(variant.kvHeads)] : null,
    variant.headDim ? ["Head dim", fmtNumber(variant.headDim)] : null,
    variant.keyLength || variant.valueLength ? ["K / V 长度", `${fmtNumber(variant.keyLength || variant.headDim)} / ${fmtNumber(variant.valueLength || variant.headDim)}`] : null,
    variant.contextLength ? ["原生上下文", fmtTokens(variant.contextLength)] : null,
    variant.slidingWindow ? ["Sliding window", fmtTokens(variant.slidingWindow)] : null,
    variant.slidingLayers ? ["Sliding / global 层", `${fmtNumber(variant.slidingLayers)} / ${fmtNumber(variant.globalLayers || 0)}`] : null,
    variant.shardCount > 1 ? ["权重分片", `${fmtNumber(variant.shardCount)}${variant.complete ? " · 完整" : " · 不完整"}`] : null,
  ].filter(Boolean);
  const componentRows = components.map((component) => `<span class="gguf-component gguf-component-${escapeAttr(component.kind)}" title="${escapeAttr(component.path)}"><i data-lucide="${component.kind === "mmproj" ? "image" : component.kind === "dflash" ? "zap" : "puzzle"}"></i><strong>${escapeHtml(component.kind === "mmproj" ? "MMProj（可选视觉）" : component.kind === "dflash" ? "DFlash" : "组件")}</strong>${escapeHtml(component.name)}${component.fileBytes ? ` · ${fmtBytes(component.fileBytes)}` : ""}</span>`).join("");
  root.innerHTML = `
    <div class="selected-model-head">
      <div><span>当前主变体</span><strong>${escapeHtml(variant.name)}</strong><code>${escapeHtml(variant.launchModel)}</code></div>
      <span class="runtime-compat runtime-compat-${escapeAttr(compatibility.state)}" title="${escapeAttr(compatibility.detail)}"><i data-lucide="${compatibility.state === "ok" ? "badge-check" : compatibility.state === "fail" ? "circle-x" : compatibility.state === "warn" ? "triangle-alert" : "circle-help"}"></i>${escapeHtml(compatibility.label)}</span>
    </div>
    ${metrics.length ? `<div class="selected-model-metrics">${metrics.map(([label, value]) => `<div><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("")}</div>` : '<div class="selection-hint">该文件尚无可用 GGUF 元数据；显存估算会回退到文件大小和安全余量。</div>'}
    <div class="gguf-components">${componentRows || '<span class="gguf-components-empty">没有发现 mmproj / DFlash 附属组件。</span>'}</div>
    <div class="selected-model-footer">
      <span>${escapeHtml(compatibility.detail || "缺字段时不会猜测兼容性。")}</span>
      <button type="button" class="ghost-mini-button" data-apply-selected-recommendations ${compatibility.state === "fail" ? "disabled" : ""}><i data-lucide="wand-sparkles"></i><span>应用推荐参数</span></button>
    </div>
  `;
  renderIcons();
}

function applyGgufRecommendations(selected, options = {}) {
  if (!selected?.variant) return [];
  const { variant, components } = selected;
  ...
  const setValue = (selector, value, label) => {
    ...
    applied.push(label || selector);
  };
  const nativeContext = Number(variant.contextLength || 0);
  const recommendedContext = Number(recommendations.maxModelLen || recommendations.contextLength || recommendations.maxContextLength || 0);
  const context = recommendedContext || (nativeContext ? Math.min(nativeContext, 131072) : 0);
  if (context) setValue("#maxModelLen", context, `上下文 ${fmtTokens(context)}`);
  setValue("#maxNumSeqs", recommendations.maxNumSeqs || recommendations.parallelSlots, "并行槽");
  setValue("#gpuMemoryUtilization", recommendations.gpuMemoryUtilization, "显存比例");
  setValue("#gpuLayers", recommendations.gpuLayers || (options.derived !== false ? "all" : ""), "GPU layers");
  setValue("#batchSize", recommendations.batchSize, "batch");
  setValue("#ubatchSize", recommendations.ubatchSize, "ubatch");
  const fallbackCache = context >= 131072 ? "q4_0" : context >= 65536 ? "q8_0" : "";
  setValue("#cacheTypeK", recommendations.cacheTypeK || recommendations.kvCacheDtype || fallbackCache, "K cache");
  setValue("#cacheTypeV", recommendations.cacheTypeV || recommendations.kvCacheDtype || fallbackCache, "V cache");
  setValue("#flashAttention", recommendations.flashAttention, "Flash Attention");
  setValue("#multiGpuMode", recommendations.multiGpuMode, "多 GPU 模式");
  setValue("#tensorSplit", recommendations.tensorSplit, "tensor split");
  if (recommendations.textOnlyMode !== undefined) setValue("#textOnlyMode", recommendations.textOnlyMode, "文本/多模态");
  const mmproj = recommendations.mmproj || recommendations.availableMmproj || components.find((component) => component.kind === "mmproj")?.path;
  if ($("#textOnlyMode")?.checked) {
    const projectorInput = $("#mmproj");
    if (projectorInput) projectorInput.value = "";
  } else if (mmproj) {
    setValue("#mmproj", mmproj, "MMProj");
  }
  const dflash = recommendations.dflash || recommendations.draftModel || components.find((component) => component.kind === "dflash")?.path;
  if (dflash) {
    setValue("#dflash", dflash, "DFlash");
    setValue("#speculativeMode", recommendations.speculativeMode || "draft-dflash", "推测解码");
  } else {
    setValue("#speculativeMode", recommendations.speculativeMode, "推测解码");
  }
  setValue("#numSpeculativeTokens", recommendations.numSpeculativeTokens, "推测 token");
  updateTextOnlyNote();
...
function autoTuneLlamaLaunch() {
  const selected = getSelectedGgufInfo();
  if (!selected) {
    notify("无法应用模型推荐", "请先从模型库或选择器中选中一个具体 GGUF 变体。", "info");
    return;
  }
  if (selected.variant.compatibility?.state === "fail") {
    notify("当前运行时不兼容", selected.variant.compatibility.detail || selected.variant.name, "error");
    return;
  }
  const applied = applyGgufRecommendations(selected);
  notify("已应用模型推荐", applied.length ? applied.join(" · ") : "已保留当前配置；后端未提供额外推荐字段。", "success");
}
...
async function refreshRecentLaunches() {
  try {
    ...
    state.recentLaunches = deriveRecentLaunchesFromJobs();
  }
  renderRecentLaunches();
}

function deriveRecentLaunchesFromJobs() {
  return (state.jobs || [])
    .filter((job) => job?.type === "serve" && job?.status === "success")
    .sort((left, right) => String(right.updatedAt || right.createdAt || "").localeCompare(String(left.updatedAt || left.createdAt || "")))
    .slice(0, 5)
    .map((job) => ({
      model: job.meta?.model || job.meta?.launchModel || "",
      name: job.meta?.name || job.meta?.servedName || job.meta?.model || "最近启动",
      config: job.meta || {},
    }));
}
...
function renderRecentLaunches() {
  ...
  if (!root) return;
  const launches = state.recentLaunches?.length ? state.recentLaunches : deriveRecentLaunchesFromJobs();
  if (!launches.length) {
    root.hidden = true;
    root.innerHTML = "";
    return;
  ...
  root.hidden = false;
  root.innerHTML = '<span class="recent-launches-label">最近启动</span>' + launches.map((item, index) => {
    const label = item.name || item.config?.name || item.model;
    return `<button type="button" class="recent-launch-chip" data-recent-index="${index}" title="${escapeAttr(item.model || label)}"><i data-lucide="rotate-cw"></i><span>${escapeHtml(label)}</span></button>`;
  }).join("");
  root.__launches = launches;
  renderIcons();
}
...
function handleRecentLaunchAction(event) {
  ...
  if (!chip) return;
  const launches = $("#recentLaunches")?.__launches || state.recentLaunches || [];
  const item = launches[Number(chip.dataset.recentIndex)];
...
// Tracks whether the polled data on screen is still live. Without this every
// poll failure was swallowed and the console kept showing a frozen snapshot.
const connectionStatus = window.ConnectionStatus.create({
  getElement: $,
  escapeHtml,
  renderIcons,
  isEnglish: () => effectiveLanguage() === "en-US",
  // llama uses a phrase-map translator rather than keyed copy, so the strings
  // are picked here instead of going through a lookup.
  label: (english) => (english ? "llama.cpp manager" : "llama.cpp 管理器"),
  title: (english) => (english ? "Lost connection to the llama.cpp manager" : "与 llama.cpp 管理器的连接已中断"),
  onDisconnect: (message) => {
    const english = effectiveLanguage() === "en-US";
    notify(
      english ? "Lost connection to the llama.cpp manager" : "与 llama.cpp 管理器的连接已中断",
      message || (english ? "Polling has failed repeatedly; the data on screen has stopped updating." : "轮询连续失败，界面数据已停止更新。"),
      "error",
    );
  },
  onReconnect: () => {
    const english = effectiveLanguage() === "en-US";
    notify(
      english ? "Connection restored" : "连接已恢复",
      english ? "Data is updating again." : "数据重新开始更新。",
      "success",
    );
    // Pull everything fresh rather than waiting out the poll intervals.
    Promise.all([
      refreshStatus().catch(() => {}),
      refreshJobs().catch(() => {}),
      refreshModels().catch(() => {}),
...
async function loadReferenceData() {
  ...
  if (data?.gpuPerformance && window.GpuPlanningUtils?.configureGpuPerformanceTable) {
    window.GpuPlanningUtils.configureGpuPerformanceTable(data.gpuPerformance);
  }
  return data;
}
...
async function init() {
  ...
  // Updatable reference tables. Failure is non-fatal: the built-in table stays.
  ...
  try {
    ...
    // Seeds lastSuccessAt so the freshness clock starts from a real load.
    ...
    // status/jobs 走 connectionStatus.track，失败会累计并触发断连横幅
    setInterval(() => {
      if (!document.hidden) connectionStatus.track(refreshStatus());
    }, 5000);
    const pollJobs = () => {
      if (!document.hidden) connectionStatus.track(refreshJobs());
    };
    let jobsTimer = setInterval(pollJobs, 3000);
    window.__adaptJobsPoll = (jobs = []) => {
      const active = (Array.isArray(jobs) ? jobs : []).some((job) => ["running", "queued"].includes(job.status));
      clearInterval(jobsTimer);
      jobsTimer = setInterval(pollJobs, active ? 1000 : 10000);
    };
    setInterval(() => {
    ...
    setInterval(() => {
      if (!document.hidden && activeViewName === "logs") refreshLogs().catch(() => {});
    }, 3000);
  } catch (error) {
    // The banner covers the ongoing case; the first load needs it too, so a
    // console opened against a dead manager says so instead of sitting empty.
    connectionStatus.markFailure(error);
...
function bindEvents() {
  ...
  $("#localModelSearch")?.addEventListener("input", renderModels);
  $("#localModelCompatibility")?.addEventListener("change", renderModels);
  ...
  $("#autoTuneBtn")?.addEventListener("click", autoTuneLlamaLaunch);
  $("#recentLaunches")?.addEventListener("click", handleRecentLaunchAction);
  $("#selectedModelDetails")?.addEventListener("click", (event) => {
    if (!event.target.closest("[data-apply-selected-recommendations]")) return;
    autoTuneLlamaLaunch();
  });
  ...
  $("#llamaApiKey")?.addEventListener("input", renderNetworkNote);
  ["#startModel", "#maxModelLen", "#maxNumSeqs", "#gpuMemoryUtilization", "#gpuLayers", "#batchSize", "#ubatchSize", "#loadFormat", "#cacheTypeK", "#cacheTypeV", "#multiGpuMode", "#tensorSplit", "#mainGpu", "#textOnlyMode", "#mmproj", "#dflash", "#speculativeMode", "#numSpeculativeTokens"].forEach((selector) => {
    const node = $(selector);
    if (!node) return;
  ...
  $("#textOnlyMode")?.addEventListener("change", handleTextOnlyModeChange);
  ...
  $("#memoryEstimateNote")?.addEventListener("click", (event) => {
    ...
    updateMemoryEstimate();
    notify("已应用建议上下文", `${fmtTokens(Number(button.dataset.applyContext || 0))} tokens`, "success");
  });
  ...
  $("#downloadHfMirror")?.addEventListener("change", saveDownloadQueueMode);
  $("#downloadHfTransfer")?.addEventListener("change", saveDownloadQueueMode);
...
const languageTranslationRoots = new Set();
const originalLanguageText = new WeakMap();
const originalLanguageAttributes = new WeakMap();

function initLanguageObserver() {
  if (languageObserver || !document.body) return;
  languageObserver = new MutationObserver((records) => {
    records.forEach((record) => {
      if (record.type === "characterData" && record.target.parentElement) languageTranslationRoots.add(record.target.parentElement);
      record.addedNodes?.forEach((node) => {
        if (node.nodeType === Node.ELEMENT_NODE) languageTranslationRoots.add(node);
        else if (node.parentElement) languageTranslationRoots.add(node.parentElement);
      });
    });
...
function scheduleLanguageTranslation() {
  ...
  const flush = () => {
    ...
    const roots = Array.from(languageTranslationRoots);
    languageTranslationRoots.clear();
    roots.filter((root) => root?.isConnected).forEach((root) => {
      translateVisibleText(root);
      translateUiAttributes(root);
    });
  };
...
function applyLanguageMode() {
  ...
  document.title = `llama.cpp Manager - ${t("app.title")}`;
  document.body?.setAttribute("data-language", mode);
  if ($("#languageMode")) $("#languageMode").value = state.uiPrefs.language;
  setUiText(".brand p", t("brand.subtitle"));
  setUiText(".topbar h2", t("app.title"));
  applyNavLabel("service", t("nav.service"));
  applyNavLabel("models", t("nav.models"));
  applyNavLabel("download", t("nav.download"));
  applyNavLabel("exposure", t("nav.exposure"));
  applyNavLabel("external-access", t("nav.externalAccess"));
  applyNavLabel("billing", t("nav.billing"));
  applyNavLabel("tools", t("nav.tools"));
  applyNavLabel("stats", t("nav.stats"));
  applyNavLabel("audit", t("nav.audit"));
  applyNavLabel("logs", t("nav.logs"));
...
function translateVisibleText(root = document.body) {
  if (!root) return;
  const english = effectiveLanguage() === "en-US";
  const dictionary = english ? EN_TEXT : ZH_TEXT;
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (!english && originalLanguageText.has(node)) return NodeFilter.FILTER_ACCEPT;
      return translateLlamaPhrase(node.nodeValue.trim(), dictionary) !== node.nodeValue.trim()
  ...
  nodes.forEach((node) => {
    ...
    const original = originalLanguageText.get(node) || raw.trim();
    if (english) {
      originalLanguageText.set(node, original);
      const translated = translateLlamaPhrase(original, dictionary);
      if (translated && translated !== raw.trim()) node.nodeValue = `${leading}${translated}${trailing}`;
    } else if (originalLanguageText.has(node)) {
      node.nodeValue = `${leading}${original}${trailing}`;
    } else {
      const translated = translateLlamaPhrase(raw.trim(), dictionary);
      if (translated && translated !== raw.trim()) node.nodeValue = `${leading}${translated}${trailing}`;
    }
  });
}

function translateLlamaPhrase(value, dictionary) {
  const text = String(value || "").trim();
  if (!text) return text;
  if (dictionary[text]) return dictionary[text];
  if (effectiveLanguage() !== "en-US") return text;
  let translated = text;
  for (const [pattern, replacement] of UI_TRANSLATION_PATTERNS || []) {
    pattern.lastIndex = 0;
    translated = translated.replace(pattern, replacement);
  }
  translated = (UI_PARTIAL_TRANSLATIONS || []).reduce((current, entry) => current.replace(entry[0], entry[1]), translated);
  return dictionary[translated] || translated;
}

function translateUiAttributes(root = document.body) {
  ...
  const elements = [root, ...Array.from(root?.querySelectorAll?.("*") || [])].filter((node) => node instanceof Element);
  const dictionary = en ? EN_TEXT : ZH_TEXT;
  elements.forEach((element) => {
    if (element.closest("script, style, code, pre, .logs-box, .audit-markdown, [data-no-i18n]")) return;
    ["title", "placeholder", "aria-label"].forEach((attribute) => {
      if (!element.hasAttribute(attribute)) return;
      const previous = originalLanguageAttributes.get(element) || {};
      const current = element.getAttribute(attribute) || "";
      const original = previous[attribute] || current;
      if (en) {
        originalLanguageAttributes.set(element, { ...previous, [attribute]: original });
        const translated = translateLlamaPhrase(original, dictionary);
        if (translated !== current) element.setAttribute(attribute, translated);
      } else if (previous[attribute] && current !== previous[attribute]) {
        element.setAttribute(attribute, previous[attribute]);
      }
    });
  });
  syncNavLabels();
...
function applyNavLabel(view, label) {
  const link = $(`.nav [data-view='${view}'], .nav [data-nav='${view}']`);
...
// Below 820px the nav label span is hidden and only the icon remains, so the
// tooltip and accessible name have to carry the label. The visible text is
// already translated by translateVisibleText; mirror whatever it ended up as.
function syncNavLabels() {
  document.querySelectorAll(".nav [data-view], .nav [data-nav]").forEach((link) => {
...
function showView(view, updateHash = true) {
  ...
  if (next === "logs") refreshLogs().catch(() => {});
  if (next === "audit") {
    refreshAuditStatus().catch(() => {});
...
function updateDownloadPlanPreview(extra = {}) {
  if (extra && (extra.bytes || extra.expectedBytes || extra.fileCount)) {
    state.downloadEstimate = extra;
  }
...
const refreshJobs = withFlightGuard(async () => {
  state.jobs = await api("/api/jobs?fields=summary");
  renderJobs();
  renderModels();
  renderRecentLaunches();
  maybeHydrateLaunchFormFromRuntime();
  window.__adaptJobsPoll?.(state.jobs);
});
...
async function loadDownloadSettings() {
  try {
    ...
    if ($("#downloadHfMirror")) $("#downloadHfMirror").checked = Boolean(data?.hfMirror);
    if ($("#downloadHfTransfer")) $("#downloadHfTransfer").checked = Boolean(data?.hfTransfer);
  } catch {
    // 设置读取失败不阻塞页面初始化
  }
}
...
async function saveDownloadQueueMode() {
  ...
  try {
    const settings = {
      ...
      hfMirror: Boolean($("#downloadHfMirror")?.checked),
      hfTransfer: Boolean($("#downloadHfTransfer")?.checked),
    };
...
function renderClaudeCompressionSettings() {
  ...
  if (!$("#claudeCompressionEnabled")) return;
...
function updateClaudeCompressionNote() {
  if (!$("#claudeCompressionNote")) return;
...
async function saveClaudeCompression(event) {
  const settings = readClaudeCompressionForm();
  const percentages = [settings.triggerRatio * 100, settings.recentRatio * 100, settings.summaryRatio * 100];
  if (percentages.some((value) => !Number.isFinite(value)) || percentages[0] < 50 || percentages[0] > 99 || percentages[1] < 5 || percentages[1] > 50 || percentages[2] < 5 || percentages[2] > 50) {
    notify("压缩参数无效", "触发阈值需为 50–99%，最近原文和摘要预算需为 5–50%。", "error");
    return;
  }
...
async function setupClaudeBridge(event) {
  const clearBusy = setButtonBusy(event.currentTarget, "配置中...");
  try {
    const result = await api("/api/claude/setup", { method: "POST", body: "{}" });
...
function getLaunchFormConfig() {
  ...
  payload.cacheReuse = Number(form.get("cacheReuse") || $("#cacheReuse")?.value || 512);
...
function applyLaunchProfile(config = {}, options = {}) {
  ...
  set("#cacheReuse", config.cacheReuse ?? 512);
  ...
  set("#reasoningEffort", config.reasoningEffort);
  set("#reasoningBudget", config.reasoningBudget);
  set("#multiGpuMode", config.multiGpuMode);
  set("#tensorSplit", config.tensorSplit);
  set("#mainGpu", config.mainGpu);
  set("#mmproj", config.mmproj);
  set("#mmprojDevice", config.mmprojDevice);
  set("#dflash", config.dflash || config.draftModel);
...
function renderRuntimeFacts(status = state.status) {
  window.LlamaRuntimeInsights?.renderRuntimeFacts(status, {
    formatContextUsage,
    fmtTokens,
    escapeHtml,
  });
...
function renderLlamaMemoryEstimate(estimate) {
  ...
  root.innerHTML = [
    ...
    { label: "GPU 权重", value: `${formatGbNumber(estimate.weightsGb)} GB`, detail: `${estimate.metadataSource || estimate.quantLabel} · 总 ${formatGbNumber(estimate.totalWeightsGb)} GB` },
...
function buildLlamaMemoryEstimatePayload(estimate) {
  const selected = getSelectedGgufInfo();
  const selectedMmproj = selected?.components?.find((component) => component.kind === "mmproj");
  const selectedDraft = selected?.components?.find((component) => component.kind === "dflash");
  const explicitMmproj = $("#textOnlyMode")?.checked ? "" : $("#mmproj")?.value || "";
  const textOnlyMode = Boolean($("#textOnlyMode")?.checked);
  const explicitDraft = $("#dflash")?.value || "";
  const speculativeMode = effectiveLlamaSpeculativeModeForEstimate();
  const draftComponent = speculativeMode.startsWith("draft-")
    ? explicitDraft ? { ...(selectedDraft || {}), path: explicitDraft } : selectedDraft || null
    : null;
  return {
    model: $("#startModel")?.value || "",
    variant: selected?.variant?.raw || selected?.variant || null,
    paramsB: estimate.paramsB,
    contextTokens: estimate.contextTokens,
    bytesPerParam: estimate.bytesPerParam,
    kvBytes: estimate.kvBytes,
    cacheTypeK: $("#cacheTypeK")?.value || "f16",
    cacheTypeV: $("#cacheTypeV")?.value || "f16",
    arch: estimate.arch,
    ...
    // Always send the discovered projector as availability metadata. The backend
    // counts it only when textOnlyMode=false, so the estimate and optional UI stay in sync.
    mmproj: explicitMmproj ? { ...(selectedMmproj || {}), path: explicitMmproj } : selectedMmproj || null,
    dflash: draftComponent,
...
function mergeLlamaServerMemoryEstimate(estimate, result) {
  state.memoryEstimateServer = {
    model: $("#startModel")?.value || "",
    metadata: result?.metadata || null,
    selectedVariant: result?.selectedVariant || null,
    components: result?.components || [],
    recommendedConfig: result?.recommendedConfig || null,
  };
  renderSelectedModelDetails();
  ...
  return {
    ...estimate,
    serverBacked: true,
    estimateConfidence: result?.metadata ? "high" : estimate.estimateConfidence,
    metadataSource: result?.metadata ? "GGUF metadata + 后端校准" : estimate.metadataSource,
    ...
    draftReserveGb: plan.speculativeReserveGb ?? estimate.draftReserveGb,
...
function updateGgufModeState() {
  ...
  const model = $("#startModel")?.value.trim() || "";
  const selected = getSelectedGgufInfo();
  const local = selected?.model || getLocalModelForInput(model);
  const selectedFile = selected?.variant;
  const shardText = selectedFile?.shardCount > 1 ? `（${selectedFile.shardCount} 个分片）` : "";
  const fileText = selectedFile
    ? `将使用主变体 ${selectedFile.name || selectedFile.path}${shardText}。`
...
function updateTextOnlyNote() {
  ...
  const en = effectiveLanguage() === "en-US";
  const hasMmproj = Boolean(getSelectedGgufInfo()?.components?.some((component) => component.kind === "mmproj"));
  const multimodalHint = hasMmproj || isLikelyMultimodalModel(model)
    ? en
      ? " An optional mmproj is available or likely; it is loaded only after text-only mode is turned off."
      : " 检测到可选 mmproj（或模型可能支持视觉）；只有关闭仅文本模式后才会加载。"
    : "";
  const title = enabled
...
function handleTextOnlyModeChange() {
  const textOnly = $("#textOnlyMode")?.checked !== false;
  const mmprojInput = $("#mmproj");
  if (!textOnly && mmprojInput && !mmprojInput.value) {
    const projector = getSelectedGgufInfo()?.components?.find((component) => component.kind === "mmproj");
    if (projector?.path) mmprojInput.value = projector.path;
  }
  updateTextOnlyNote();
  updateMemoryEstimate();
}
...
function getLocalModelForInput(model) {
  ...
  if (!key) return null;
  return [...(state.models.local || []), ...(state.models.cached || [])].find((item) => {
    return normalizePathKey(item.launchModel) === key
      ...
      || ggufVariantsFor(item).some((variant) => normalizePathKey(variant.launchModel || variant.path) === key);
  }) || null;
}
...
function estimateMemoryUsage(contextOverride = null) {
  const model = $("#startModel")?.value.trim() || "";
  const contextTokens = Math.max(512, Number(contextOverride ?? $("#maxModelLen")?.value ?? 8192));
  ...
  const paramsB = Number(variant?.paramsB || 0) || inferParamBillions(model);
  const dtype = "f16";
  ...
  const exactWeightBytes = Number(variant?.fileBytes || 0);
  const bytesPerParam = exactWeightBytes && paramsB
    ? exactWeightBytes / (paramsB * 1_000_000_000)
    : selectedQuant.bytesPerParam || dtypeBytes;
  const kvBytes = ((KV_DTYPE_BYTES[cacheTypeK] || dtypeBytes) + (KV_DTYPE_BYTES[cacheTypeV] || dtypeBytes)) / 2;
  const arch = variant?.layers && variant?.kvHeads && variant?.headDim
    ? {
        layers: variant.layers,
        kvHeads: variant.kvHeads,
        headDim: variant.headDim,
        contextLength: variant.contextLength || 0,
        slidingWindow: variant.slidingWindow || 0,
        slidingLayers: variant.slidingLayers || 0,
        globalLayers: variant.globalLayers || 0,
      }
  ...
  const totalWeightsGb = exactWeightBytes ? exactWeightBytes / 1024 ** 3 : paramsB ? paramsB * 1_000_000_000 * bytesPerParam / 1024 ** 3 : 0;
  const weightsGb = totalWeightsGb * gpuLayerRatio;
  const cpuWeightsGb = Math.max(0, totalWeightsGb - weightsGb);
  const hasSlidingBreakdown = Number(arch?.slidingWindow || 0) > 0
    && Number(arch?.slidingLayers || 0) > 0
    && Number(arch?.globalLayers || 0) >= 0
    && Number(arch.slidingLayers || 0) + Number(arch.globalLayers || 0) === Number(arch.layers || 0);
  const kvLayerTokens = hasSlidingBreakdown
    ? (Number(arch.globalLayers || 0) * contextTokens + Number(arch.slidingLayers || 0) * Math.min(contextTokens, Number(arch.slidingWindow))) * parallelSlots
    : Number(arch?.layers || 0) * totalContextTokens;
  const totalKvGb = paramsB && arch
    ? kvLayerTokens * 2 * arch.kvHeads * arch.headDim * kvBytes / 1024 ** 3
    : 0;
  ...
  const mmprojBytes = Number(selected?.components?.find((component) => component.kind === "mmproj")?.fileBytes || 0);
  const draftBytes = Number(selected?.components?.find((component) => component.kind === "dflash")?.fileBytes || 0);
  const multimodalReserveGb = !textOnlyMode
    ? mmprojBytes ? mmprojBytes / 1024 ** 3 : paramsB && isLikelyMultimodalModel(model) ? 1.5 : 0
    : 0;
  const draftReserveGb = effectiveLlamaSpeculativeModeForEstimate().startsWith("draft-") ? draftBytes / 1024 ** 3 : 0;
  const overheadGb = paramsB ? Math.max(1.2, (weightsGb + kvGb) * 0.08) + multimodalReserveGb + draftReserveGb : 0;
  ...
  const estimateConfidence = exactWeightBytes && variant?.layers && variant?.kvHeads && variant?.headDim ? "high" : paramsB && arch ? "medium" : "low";
  ...
  return {
    ...
    multimodalReserveGb,
    draftReserveGb,
    metadataSource: estimateConfidence === "high" ? "GGUF metadata" : "模型名启发式",
    paramsB,
    bytesPerParam,
    kvBytes,
    arch,
    ...
    estimateConfidence,
    status,
    nativeMaxTokens: Math.max(0, Number(variant?.contextLength || arch?.contextLength || 0)),
...
function renderMemoryEstimateNote(estimate) {
  ...
  const draftNote = estimate.draftReserveGb > 0
    ? ` DFlash/draft 组件另计约 ${formatGbNumber(estimate.draftReserveGb)} GB。`
    : "";
  const replacementBudget = estimate.gpuAllocations.some((item) => item.capacityMode === "replacement");
  const confidenceText = estimate.estimateConfidence === "high"
    ? "高：使用所选 GGUF 的真实文件大小、层数与 KV 结构"
    : estimate.estimateConfidence === "medium"
      ? "中：按参数量、GGUF 量化和常见层数/KV 结构估算"
      : "低：缺少参数量或架构信息";
  const confidenceLine = `<span class="memory-headroom">估算可信度：${confidenceText}。${replacementBudget ? "当前 llama.cpp 容器会在换模前释放，容量按替换后预算计算。" : "显存可用值按当前空闲显存减保护余量与利用率取较小值。"}</span>`;
  ...
  const serverLine = estimate.serverBacked && estimate.serverRecommendations?.suggestions?.length
    ...
    : "";
  const nativeLine = estimate.nativeMaxTokens && estimate.contextTokens > estimate.nativeMaxTokens
    ? `<span class="memory-recommend warn">当前 ${fmtTokens(estimate.contextTokens)} 已超过模型原生上下文 ${fmtTokens(estimate.nativeMaxTokens)}；只有模型与 RoPE 配置明确支持扩展时才建议继续。</span>`
    : "";
  const recommendedContext = recommendMaxContextForCurrentLaunch(estimate);
  const contextLine = recommendedContext && recommendedContext < estimate.contextTokens
    ? `<span class="memory-recommend warn">按当前 GPU、层数、KV 精度和并行槽估算，建议上下文不超过 ${fmtTokens(recommendedContext)}。 <button type="button" class="ghost-mini-button" data-apply-context="${recommendedContext}">应用建议</button></span>`
    : recommendedContext
      ? `<span class="memory-headroom">按当前配置二分估算的线内容量约 ${fmtTokens(recommendedContext)} tokens。</span>`
      : "";
  return `
    <strong class="${estimate.status}">${statusText}</strong>
    <span>${escapeHtml(note + longContextNote + textOnlyNote + draftNote)} 权重量化：${escapeHtml(estimate.quantNote)}。</span>
    ${confidenceLine}
    ${serverLine}
    ${nativeLine}
    ${contextLine}
...
function recommendMaxContextForCurrentLaunch(baseEstimate) {
  if (!baseEstimate?.paramsB) return 0;
  const hardMax = Math.min(4_194_304, Math.max(baseEstimate.contextTokens, baseEstimate.nativeMaxTokens || 1_048_576));
  let low = 512;
  let high = hardMax;
  for (let index = 0; index < 20 && high - low > 512; index += 1) {
    const middle = Math.floor(((low + high) / 2) / 512) * 512;
    const candidate = estimateMemoryUsage(middle);
    if (candidate.status === "fail") high = Math.max(512, middle - 512);
    else low = middle;
  }
  return Math.max(512, Math.floor(low / 512) * 512);
}

function updateContextPresetState(contextTokens, baseEstimate) {
  document.querySelectorAll("[data-context-preset]").forEach((button) => {
    const tokens = Number(button.dataset.contextPreset);
    const candidate = baseEstimate?.paramsB ? estimateMemoryUsage(tokens) : null;
    button.classList.toggle("active", tokens === Number(contextTokens));
    ["fit-ok", "fit-warn", "fit-fail"].forEach((name) => button.classList.remove(name));
    if (candidate) button.classList.add(`fit-${candidate.status}`);
    const beyondNative = Boolean(baseEstimate?.nativeMaxTokens && tokens > baseEstimate.nativeMaxTokens);
    button.classList.toggle("beyond-native", beyondNative);
    button.title = candidate
      ? `${fmtTokens(tokens)} · ${candidate.status === "ok" ? "预计可运行" : candidate.status === "warn" ? "接近显存上限" : "预计超显存"}${beyondNative ? " · 超过模型原生上下文" : ""}`
...
function renderNetworkNote() {
  ...
  const apiKeyFilled = Boolean($("#llamaApiKey")?.value.trim());
...
async function runPortCheck() {
  ...
  if (!Number.isInteger(port) || port < 1024 || port > 65535) {
    el.textContent = "请输入 1024–65535 的端口";
  ...
  try {
    ...
    if (result.available) {
      ...
      el.dataset.state = "ok";
      el.textContent = `端口 ${port} 当前由本管理器模型使用，替换启动时会安全切换`;
    } else {
      el.dataset.state = "fail";
...
async function saveServiceExposure(event) {
  ...
  if (payload.apiKey && $("#llamaApiKey")) $("#llamaApiKey").value = payload.apiKey;
  renderServiceExposure();
...
function applyExposureToLaunchForm() {
  ...
  if (typedKey && $("#llamaApiKey")) $("#llamaApiKey").value = typedKey;
  renderNetworkNote();
...
function renderCacheHitStats(stats) {
  const root = $("#cacheHitSummary");
  const note = $("#cacheHitNote");
  if (!root || !note) return;
  const cache = stats?.cacheHit || {};
  const en = effectiveLanguage() === "en-US";
  const labels = en ? {
    latest: "Latest request",
    hour: "Last hour",
    cumulative: "Since enabled",
    waiting: "Waiting for an observed request",
    noCompleted: "No completed request observed in this window",
    reused: "input tokens reused",
    processed: "processed",
  ...
  root.innerHTML = [
    cacheHitMetric(labels.latest, cache.latest, labels, true),
    cacheHitMetric(labels.hour, cache.rollingHour, labels),
    cacheHitMetric(labels.cumulative, cache.cumulative, labels),
  ].join("");

  const since = cache.trackingSince ? formatDateTime(cache.trackingSince) : "-";
  const observedCount = Number(cache.observedRequests || 0);
  const observed = fmtTokens(observedCount);
  const scope = en
    ? `Tracking since ${since}; ${observed} completed ${observedCount === 1 ? "request" : "requests"} persisted. A running latest request is not added to the hourly or cumulative totals until it completes. Very short requests can finish between polls.`
    : `从 ${since} 起已持久化 ${observed} 个完成请求。最近一次若为“生成中”，完成后才计入 1 小时和累计；极短请求可能在两次轮询之间遗漏。`;
  note.textContent = cache.error
    ? `${en ? "Cache observation is temporarily unavailable" : "缓存观测暂不可用"}：${cache.error}。${scope}`
    : scope;
}

function cacheHitMetric(label, bucket, labels, latest = false) {
  const promptTokens = Number(bucket?.promptTokens || 0);
  if (!bucket || !promptTokens) {
    const detail = latest ? labels.waiting : labels.noCompleted;
    return statsMetric(label, "-", detail);
  }
  const cachedTokens = Math.min(promptTokens, Number(bucket.cachedTokens || 0));
  const processedTokens = Math.min(promptTokens, Number(bucket.processedTokens || 0));
  const hitRate = Number.isFinite(Number(bucket.hitRate)) ? Number(bucket.hitRate) : cachedTokens / promptTokens;
  const rate = `${(Math.max(0, Math.min(1, hitRate)) * 100).toFixed(hitRate >= 0.99 ? 2 : 1)}%`;
  const status = latest
    ? ` · ${bucket.status === "running" ? labels.running : labels.completed}`
    : ` · ${fmtTokens(bucket.requests || 0)} ${labels.requests}`;
  return statsMetric(
    label,
    rate,
    `${fmtTokens(cachedTokens)} / ${fmtTokens(promptTokens)} ${labels.reused} · ${labels.processed} ${fmtTokens(processedTokens)}${status}`,
  );
}

function renderStatsFreshness(stats) {
  const element = $("#statsFreshness");
  if (!element) return;
  const updatedAt = stats.updatedAt || stats.generatedAt || stats.timestamp;
  if (!updatedAt) {
    element.textContent = "实时轮询";
    return;
  }
  ...
  element.textContent = ageSec < 5 ? "刚刚更新" : `更新于 ${ageSec} 秒前`;
}
...
function renderStatsComposition(stats) {
  ...
  const prompt = Number(tokens.prompt || tokens.input || 0);
  const cacheObserved = Number(stats.cacheHit?.cumulative?.cachedTokens || 0);
  const cached = Math.min(prompt, Number(tokens.cachedPrompt || tokens.cached || cacheObserved || 0));
  const uncached = Math.max(0, prompt - cached);
  const generation = Number(tokens.generation || tokens.output || 0);
  const totalTokens = uncached + cached + generation;
  const requestTotal = Number(requests.total || 0);
  if (!totalTokens && !requestTotal) {
    root.innerHTML = '<div class="empty compact">暂无负载数据。产生请求后会显示 token 构成和请求结果分布。</div>';
    return;
  }
  root.innerHTML = [
    renderStackedComposition("Token 构成", [
      { label: "未缓存输入", value: uncached, color: "var(--blue)" },
      { label: "缓存复用输入", value: cached, color: "var(--teal)" },
      { label: "输出生成", value: generation, color: "var(--amber)" },
    ], totalTokens, fmtTokens),
    renderStackedComposition("请求结果", [
      { label: "成功", value: Number(requests.success || requests.completed || 0), color: "var(--green)" },
      { label: "错误", value: Number(requests.error || requests.failed || 0), color: "var(--red)" },
      { label: "中止", value: Number(requests.aborted || requests.cancelled || 0), color: "var(--amber)" },
    ], requestTotal, fmtTokens),
  ].join("");
}

function renderStackedComposition(title, segments, total, formatter) {
  const safeTotal = Math.max(1, total);
  const bar = segments.filter((segment) => segment.value > 0).map((segment) => `<div class="stack-seg" style="width:${(segment.value / safeTotal * 100).toFixed(2)}%;background:${segment.color}" title="${escapeAttr(segment.label)}: ${formatter(segment.value)}"></div>`).join("");
  const legend = segments.map((segment) => `<div class="stack-legend-item"><span class="stack-dot" style="background:${segment.color}"></span>${escapeHtml(segment.label)} · ${formatter(segment.value)} · ${fmtPct(total ? segment.value / total : 0)}</div>`).join("");
  return `
    <div class="composition-card">
      <div class="composition-title"><span>${escapeHtml(title)}</span><strong>${formatter(total)}</strong></div>
...
function renderStatsGpuDetail(stats) {
  ...
  const gpus = Array.isArray(stats.gpu?.gpus) ? stats.gpu.gpus : Array.isArray(state.status?.gpu?.gpus) ? state.status.gpu.gpus : [];
  if (!gpus.length) {
    panel.hidden = true;
    return;
  }
  ...
  root.innerHTML = gpus.map((gpu) => {
    const usedMb = Number(gpu.usedMb ?? gpu.memoryUsedMb ?? 0);
    const totalMb = Number(gpu.totalMb ?? gpu.memoryTotalMb ?? 0);
    const memPct = totalMb ? Math.min(100, Math.round(usedMb / totalMb * 100)) : 0;
    const utilPct = Math.min(100, Math.max(0, Number(gpu.util ?? gpu.utilization ?? 0)));
    const temperature = Number(gpu.temp ?? gpu.temperature);
    const tempState = temperature >= 85 ? "fail" : temperature >= 75 ? "warn" : "ok";
    const memState = memPct >= 95 ? "fail" : memPct >= 85 ? "warn" : "ok";
    const power = Number(gpu.powerWatts ?? gpu.power);
    const powerLimit = Number(gpu.powerLimitWatts ?? gpu.powerLimit);
    const fan = Number(gpu.fanPercent ?? gpu.fan);
    return `
      <div class="gpu-detail-card">
        <div class="gpu-detail-head"><strong>GPU ${escapeHtml(String(gpu.id ?? gpu.index ?? "0"))}</strong><span>${escapeHtml(gpu.name || "NVIDIA")}</span></div>
        <div class="gpu-detail-bar-row"><span>利用率</span><div class="gpu-detail-track"><div class="gpu-detail-fill" style="width:${utilPct}%"></div></div><em>${utilPct}%</em></div>
        <div class="gpu-detail-bar-row"><span>显存</span><div class="gpu-detail-track"><div class="gpu-detail-fill ${memState}" style="width:${memPct}%"></div></div><em>${fmtNumber(usedMb)} / ${fmtNumber(totalMb)} MB</em></div>
        <div class="gpu-detail-telemetry">
          <span class="gpu-detail-temp ${tempState}">温度 ${Number.isFinite(temperature) ? `${temperature}°C` : "-"}</span>
          <span>功耗 ${Number.isFinite(power) && power > 0 ? `${power.toFixed(0)} W` : "-"}${Number.isFinite(powerLimit) && powerLimit > 0 ? ` / ${powerLimit.toFixed(0)} W` : ""}</span>
          <span>风扇 ${Number.isFinite(fan) ? `${fan.toFixed(0)}%` : "-"}</span>
        </div>
      </div>
    `;
  }).join("");
...
function buildModelPickerItems() {
  ...
  (state.models.local || []).forEach((model) => {
    ...
    const variants = ggufVariantsFor(model);
    if (variants.length) {
      variants.forEach((variant) => add({
        source: "local",
        sourceLabel: "本地 GGUF",
        label: variants.length > 1 ? `${model.label || model.id} · ${variant.name}` : model.label || model.id || variant.name,
        model: variant.launchModel,
        detail: [variant.launchModel, variant.architecture, variant.contextLength ? `${fmtTokens(variant.contextLength)} 原生上下文` : ""].filter(Boolean).join(" / "),
        format: "gguf",
        sizeBytes: variant.fileBytes || 0,
        quantLabel: variant.quant || inferModelQuantLabel([variant.name, variant.launchModel].join(" ")),
        favorite: Boolean(note?.favorite),
        runnable: variant.compatibility.state !== "fail",
        disabled: variant.compatibility.state === "fail",
        disabledReason: variant.compatibility.state === "fail" ? variant.compatibility.detail : "",
        badges: [
          "本地",
          "GGUF",
          variant.compatibility.label,
          variant.layers ? `${variant.layers} 层` : "",
          variant.kvHeads ? `${variant.kvHeads} KV heads` : "",
          ggufComponentsFor(model).some((component) => component.kind === "mmproj") ? "MMProj" : "",
          ggufComponentsFor(model).some((component) => component.kind === "dflash") ? "DFlash" : "",
          note?.favorite ? "收藏" : "",
        ],
      }));
      return;
    ...
    add({
      ...
      favorite: Boolean(note?.favorite),
      badges: ["本地", model.size ? fmtBytes(model.size) : "", model.hasGguf ? "GGUF" : "", model.hasConfig ? "config" : "", note?.favorite ? "收藏" : ""],
    });
  });
...
function renderModels() {
  const models = state.models?.local || [];
  const notes = Object.values(state.modelNotes?.notes || {});
  const noteMap = new Map(notes.map((note) => [normalizePathKey(note.model), note]));
  const result = renderGgufLibrary({
    models,
    root: $("#modelList"),
    filters: {
      search: $("#localModelSearch")?.value || "",
      compatibility: $("#localModelCompatibility")?.value || "all",
    },
    escapeHtml,
    escapeAttr,
    fmtBytes,
    fmtTokens,
    renderIcons,
    findNote: (model) => findModelNote(noteMap, model.launchModel, model.path, model.label, model.id),
    onUse: ({ model, variant }) => {
      selectLaunchModel(variant.launchModel, { name: model.label || variant.name, format: "gguf", silent: true });
    },
    onRecommend: (selected) => {
      selectLaunchModel(selected.variant.launchModel, { name: selected.model.label || selected.variant.name, format: "gguf", silent: true });
      const applied = applyGgufRecommendations(selected);
      notify("已选择变体并应用推荐", applied.join(" · ") || selected.variant.name, "success");
    },
  });
  const count = $("#localModelCount");
  if (count) count.textContent = result.visible === result.total ? `${fmtNumber(result.total)} 个目录` : `${fmtNumber(result.visible)} / ${fmtNumber(result.total)} 个目录`;
}
...
function renderRemoteModels() {
  ...
  if (state.remoteError) {
    window.remoteModelRenderer?.render({
      ...
      errorMessage: english ? `Failed to load online models: ${state.remoteError}` : `联网模型列表加载失败：${state.remoteError}`,
      escapeHtml,
    });
    return;
  }
  ...
  if (hint) {
    ...
    const source = $("#remoteSource")?.selectedOptions?.[0]?.textContent || "Hugging Face";
    const sort = $("#remoteSort")?.selectedOptions?.[0]?.textContent || "热度趋势";
    const task = $("#remoteTask")?.selectedOptions?.[0]?.textContent || "全部类型";
    ...
    hint.textContent = english
      ? `Online catalog · returned ${fmtNumber(state.remoteModels.length)}, showing ${fmtNumber(models.length)} · limit ${limit}. Sizes and formats are estimated from public metadata and GGUF filenames; gated models require a token.`
      : `${source} · ${sort} · ${task} · 已返回 ${fmtNumber(state.remoteModels.length)} 个，显示 ${fmtNumber(models.length)} 个 · ${sizeFilter} · ${quantFilter} · 上限 ${limit}${unknownHint}。列表按公开元数据和 GGUF 文件名估算，gated 模型下载前需要配置 token。`;
  }
...
async function resolveModelLink() {
  ...
  try {
    ...
    const mirrorHint = /hf-mirror\.com/i.test(url) && !$("#downloadHfMirror")?.checked
      ? " 检测到 hf-mirror 链接，建议开启“使用 HF 镜像”。"
      : "";
    $("#linkResolveResult").innerHTML = `
      <strong>${escapeHtml(result.label || result.model)}</strong>
      <span>${escapeHtml(result.source === "modelscope" ? "ModelScope" : "Hugging Face")} · ${escapeHtml(result.summary || "已解析成功，可以开始下载。")}${escapeHtml(mirrorHint)}</span>
    `;
    if (mirrorHint && $("#downloadHfMirror")) $("#downloadHfMirror").checked = true;
    return result;
  } catch (error) {
...
async function downloadModel(event) {
  ...
  try {
    ...
    const estimate = state.downloadEstimate || {};
    await api("/api/download", {
      ...
      body: JSON.stringify({
        ...
        precision,
        ...(hfToken ? { hfToken } : {}),
        ...(Number(estimate.bytes || estimate.expectedBytes || 0) > 0 ? {
          expectedBytes: Number(estimate.bytes || estimate.expectedBytes),
          expectedFiles: Number(estimate.fileCount || estimate.expectedFiles || 0) || undefined,
        } : {}),
      }),
    });
...
async function startService(event) {
  event.preventDefault();
  if (!validateLlamaLaunchForm(event.currentTarget)) return;
...
function validateLlamaLaunchForm(form) {
  ...
  const fail = (control, message) => {
    ...
    notify("启动参数无效", message, "error");
    return false;
  };
  const integer = (selector, label, min, max) => {
    const control = $(selector);
    const value = Number(control?.value);
    return Number.isInteger(value) && value >= min && value <= max
      ? true
      : fail(control, `${label}必须是 ${min}–${max} 的整数。`);
  };
  if (!form.checkValidity()) {
    form.reportValidity();
    return false;
  }
  if (!integer("#servicePort", "端口", 1024, 65535)) return false;
  if (!integer("#maxModelLen", "每槽上下文长度", 1, 4_194_304)) return false;
  if (!integer("#maxNumSeqs", "并行槽数", 1, 1024)) return false;
  if (!integer("#batchSize", "batch", 1, 1_048_576)) return false;
  if (!integer("#ubatchSize", "ubatch", 1, 1_048_576)) return false;
  if (!integer("#reasoningBudget", "思考 token 预算", -1, 4_194_304)) return false;
  if (Number($("#ubatchSize").value) > Number($("#batchSize").value)) return fail($("#ubatchSize"), "ubatch 不能大于 batch。");
  const utilization = Number($("#gpuMemoryUtilization")?.value);
  if (!Number.isFinite(utilization) || utilization < 0.1 || utilization > 0.98) return fail($("#gpuMemoryUtilization"), "显存分配比例必须在 0.1–0.98 之间。");
  const gpuLayers = String($("#gpuLayers")?.value || "").trim().toLowerCase();
  if (!/^(?:auto|all|\d+)$/.test(gpuLayers)) return fail($("#gpuLayers"), "GPU layers 只能填写 auto、all 或非负整数。");
  const split = String($("#tensorSplit")?.value || "").trim();
  if (split) {
    const parts = split.split(/[\s,]+/).filter(Boolean);
    const values = parts.map(Number);
    if (values.some((value) => !Number.isFinite(value) || value < 0) || !values.some((value) => value > 0)) {
      return fail($("#tensorSplit"), "tensor split 必须是逗号分隔的非负数字，并且至少一项大于 0。");
    }
    const selectedGpuCount = form.querySelectorAll("input[name='gpuDeviceIds']:checked").length;
    if (selectedGpuCount > 1 && $("#multiGpuMode")?.value !== "none" && values.length !== selectedGpuCount) {
      return fail($("#tensorSplit"), `tensor split 需要 ${selectedGpuCount} 项，与已选 GPU 数一致。`);
    }
  }
  return true;
}
...
function buildLlamaLaunchPayload(formElement) {
  ...
  payload.reasoningEffort = String(payload.reasoningEffort || "default");
  payload.reasoningBudget = Number(payload.reasoningBudget ?? -1);
  ...
  payload.mmprojDevice = String(payload.mmprojDevice || "auto").trim() || "auto";
  payload.dflash = String(payload.dflash || "");
  payload.speculativeMode = String(payload.speculativeMode || "auto");
  const useExternalDraft = payload.speculativeMode === "auto" || payload.speculativeMode === "draft-dflash";
  payload.draftModel = useExternalDraft ? payload.dflash : "";
  if (!useExternalDraft) payload.dflash = "";
...
async function handleRunningModelAction(event) {
  ...
  try {
    if (action === "unload-model") {
      ...
      notify("模型已卸载", result.unloaded || button.dataset.model || "llama.cpp", "success");
      await Promise.all([refreshStatus(), refreshLogs()]);
      if (state.auditToken) refreshAuditExports().catch(() => {});
    }
  } catch (error) {
    reportActionError("卸载模型失败", error);
  } finally {
    button.disabled = false;
...
async function handleServiceJobAction(event) {
  ...
  try {
    ...
    if (action === "retry-serve") {
      ...
      notify("已重新创建启动任务", payload.name || payload.model || "llama.cpp", "success");
      await refreshJobs();
    }
