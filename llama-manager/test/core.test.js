...
test("signed service-entry instance selection recognizes primary, label, and container aliases", () => {
  ...
  assert.equal(manager.runtimeMatchesServiceGatewayInstance({
    container: { name: "llama-local-vision", labels: { "ai.manager.instance": "vision" } },
  }, "vision"), true);
  assert.equal(manager.runtimeMatchesServiceGatewayInstance({
    container: { name: "llama-local-vision", labels: { "ai.manager.instance": "vision" } },
  }, "audio"), false);
...
test("runtime compatibility keeps retired Muse blocked without deleting rollback metadata", () => {
  const mutableTagInfo = manager.resolveConfiguredLlamaImageBuildInfo({
    museGlimmer: true,
    image: "local/llama.cpp:server-cuda-muse-62bf73d",
    imageInspectText: "null",
  });
  const labeledImageInfo = manager.resolveConfiguredLlamaImageBuildInfo({
    museGlimmer: true,
    image: "local/llama.cpp:server-cuda-muse-62bf73d",
    imageInspectText: JSON.stringify({
      "org.opencontainers.image.revision": "62bf73d25c53b8161f8a22894d4f90c4aebbd7d0",
    }),
  });
  const oldRuntime = manager.evaluateLlamaArchitectureRuntimeCompatibility("muse-glimmer", {
    image: "llama:old",
    build: "b10326",
    buildNumber: 10326,
    revision: "3653e6d6d547",
  });
  const stillUnsupportedRuntime = manager.evaluateLlamaArchitectureRuntimeCompatibility("muse-glimmer", {
    image: "llama:b10344",
    build: "b10344",
    buildNumber: 10344,
    revision: "7a20",
  });
  const supportedRuntime = manager.evaluateLlamaArchitectureRuntimeCompatibility("muse-glimmer", {
    image: "llama:muse",
    build: "custom",
    revision: "62bf73d25c53b8161f8a22894d4f90c4aebbd7d0",
  });
  const missingRuntime = manager.evaluateLlamaArchitectureRuntimeCompatibility("muse-glimmer", {
    image: "local/llama.cpp:server-cuda-muse-62bf73d",
    imagePresent: false,
  });
  const mutableTagWithoutRevision = manager.evaluateLlamaArchitectureRuntimeCompatibility("muse-glimmer", mutableTagInfo);
  const labeledImage = manager.evaluateLlamaArchitectureRuntimeCompatibility("muse-glimmer", labeledImageInfo);
  assert.equal(oldRuntime.status, "unsupported");
  assert.equal(oldRuntime.supported, false);
  assert.match(oldRuntime.message, /已从本地栈弃用/);
  assert.equal(stillUnsupportedRuntime.status, "unsupported");
  assert.equal(stillUnsupportedRuntime.supported, false);
  assert.equal(supportedRuntime.status, "unsupported");
  assert.equal(supportedRuntime.supported, false);
  assert.equal(missingRuntime.status, "unsupported");
  assert.equal(missingRuntime.supported, false);
  assert.match(missingRuntime.message, /不会由此检查删除/);
  assert.equal(mutableTagWithoutRevision.status, "unsupported");
  assert.equal(mutableTagWithoutRevision.supported, false);
  assert.equal(mutableTagInfo.revision, "");
  assert.equal(labeledImage.status, "unsupported");
  assert.equal(labeledImage.supported, false);
  assert.equal(manager.evaluateLlamaArchitectureRuntimeCompatibility("llama", {}).status, "not-required");
});

test("memory estimate consumes GGUF variant metadata and component bytes", () => {
  const variant = {
    path: "D:/AI/models/muse/model.gguf",
    architecture: "muse-glimmer",
    fileBytes: 19_653_957_984,
    parameterCount: 27_854_794_240,
    paramsB: 27.85479424,
    layers: 52,
    kvLayers: 52,
    kvHeads: 2,
  ...
  const base = {
    variant,
    selectedGpus: [{ id: "0", totalMb: 96 * 1024, freeMb: 90 * 1024 }],
    maxModelLen: 131072,
    maxNumSeqs: 1,
    gpuLayers: "all",
    mmproj: { path: "D:/AI/models/muse/mmproj.gguf", fileBytes: 1_400_328_928 },
    dflash: { path: "D:/AI/models/muse/dflash.gguf", fileBytes: 1_631_205_312 },
    speculativeMode: "draft-dflash",
    textOnlyMode: false,
  };
  const f16 = manager.buildLlamaMemoryEstimate({ ...base, cacheTypeK: "f16", cacheTypeV: "f16" });
  const q8 = manager.buildLlamaMemoryEstimate({ ...base, cacheTypeK: "q8_0", cacheTypeV: "q8_0" });
  assert.equal(f16.metadata.architecture, "muse-glimmer");
  assert.equal(f16.metadata.slidingLayers, 39);
  assert.equal(f16.selectedVariant.path, variant.path);
  assert.equal(f16.recommendedConfig.speculativeMode, "draft-dflash");
  assert.equal(f16.recommendedConfig.draftModel, base.dflash.path);
  assert.equal(f16.recommendedConfig.textOnlyMode, true);
  assert.equal(f16.recommendedConfig.languageModelOnly, true);
  assert.equal(f16.recommendedConfig.mmproj, "");
  assert.equal(f16.recommendedConfig.availableMmproj, base.mmproj.path);
  assert.ok(q8.plan.totalKvGb < f16.plan.totalKvGb);
  assert.ok(f16.components.mmprojBytes > 0);
  assert.ok(f16.components.draftBytes > 0);

  const textOnly = manager.buildLlamaMemoryEstimate({ ...base, maxNumSeqs: 4, textOnlyMode: true });
  assert.equal(textOnly.components.mmprojBytes, 0);
  assert.ok(textOnly.components.availableMmprojBytes > 0);
  assert.equal(textOnly.recommendedConfig.maxNumSeqs, 1);
  assert.equal(textOnly.recommendedConfig.textOnlyMode, true);
  assert.equal(textOnly.recommendedConfig.mmproj, "");
  assert.equal(textOnly.recommendedConfig.availableMmproj, base.mmproj.path);
});

test("Muse GGUF recommendations keep 128K single-slot text mode and expose mmproj as optional", () => {
  const recommendations = manager.normalizeLaunchConfig({
    model: "D:/AI/models/muse/model.gguf",
    maxModelLen: 32768,
    maxNumSeqs: 4,
    textOnlyMode: false,
  });
  manager.applySafeGgufRuntimeRecommendations(recommendations, { contextLength: 131072 }, {
    mmproj: [{ path: "D:/AI/models/muse/mmproj.gguf" }],
  });
  assert.equal(recommendations.maxModelLen, 131072);
  assert.equal(recommendations.maxNumSeqs, 1);
  assert.equal(recommendations.textOnlyMode, true);
  assert.equal(recommendations.languageModelOnly, true);
  assert.equal(recommendations.mmproj, "");
  assert.equal(recommendations.availableMmproj, "D:/AI/models/muse/mmproj.gguf");
});

test("running llama facts are recovered from non-secret container labels", () => {
  const facts = manager.runtimeFactsFromContainerLabels({
    "ai.manager.max-model-len": "131072",
    "ai.manager.max-num-seqs": "1",
    "ai.manager.gpu-layers": "all",
    "ai.manager.cache-type-k": "q4_0",
    "ai.manager.cache-type-v": "q4_0",
    "ai.manager.speculative-mode": "draft-dflash",
    "ai.manager.reasoning-effort": "high",
    "ai.manager.reasoning-budget": "2048",
    "ai.manager.mmproj-device": "CUDA0",
    "ai.manager.text-only": "false",
    "ai.manager.mmproj": "true",
    "ai.manager.draft-model": "true",
  });
