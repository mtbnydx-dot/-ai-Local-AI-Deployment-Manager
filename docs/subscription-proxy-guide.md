...
一键启动后会直接打开 `http://127.0.0.1:5176/subscription-console.html`。账号和发布配置在同一个前端内，通过两个页签切换，切换时不会离开控制台：

- `反代账号`：API Key 与订阅 OAuth 登录。
- `服务发布`：本机、局域网和可选公网地址。
...
“服务发布”页签会始终直接显示本机和局域网的四种协议地址。当前未以 LAN 模式启动时，局域网地址标记为“地址预览 · 待启用”，并给出对应系统的重启命令。公网区域可在本机填写 HTTPS 基础地址，保存后立即生成四种公网地址；该设置只声明地址，不会自动创建域名、TLS、隧道或防火墙规则。
...
- “反代账号”页签：生成客户端 API Key，发起 Codex、Claude、Kimi、xAI 或 Antigravity 登录。
- “服务发布”页签：查看本机、局域网和可选公网的统一网关地址。
