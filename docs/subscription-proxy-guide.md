...
一键启动后打开 `http://127.0.0.1:5176/`。控制台把配置拆成两个独立页面：

- `反代账号配置`：API Key 与订阅 OAuth 登录。
- `服务发布配置`：本机、局域网和可选公网地址。
...
“服务发布配置”会始终直接显示本机和局域网的四种协议地址。当前未以 LAN 模式启动时，局域网地址标记为“地址预览 · 待启用”，并给出对应系统的重启命令。公网区域可在本机填写 HTTPS 基础地址，保存后立即生成四种公网地址；该设置只声明地址，不会自动创建域名、TLS、隧道或防火墙规则。
