(function () {
  function create(deps = {}) {
    ...
    function renderCompressionInsights() {
      ...
      const sessionLimit = window.LocalAiDomUtils.LIST_LIMITS.compact;
      const sessions = copy.showCompressionSessions
        ? `<div class="client-session-list">${allSessions.slice(0, sessionLimit).map((item) => `<span>${escapeHtml(item.label || item.id)}：${fmtTokens(item.compression?.savedTokens || 0)} saved</span>`).join("")}${allSessions.length > sessionLimit ? `<span>另有 ${allSessions.length - sessionLimit} 个会话</span>` : ""}</div>`
        : "";
      root.innerHTML = `
