(function () {
  ...
  function renderCosts(stats, options = {}) {
    ...
    root.innerHTML = `
      <div class="semantic-cost-table-wrap">
        <table class="semantic-cost-table">
          <caption>${escapeHtml(labels.priceAsOf)} ${escapeHtml(stats.pricingAsOf || "current")} ${escapeHtml(labels.publicPrice)}</caption>
          <thead><tr>
            <th scope="col">${escapeHtml(labels.model)}</th>
            <th scope="col">${escapeHtml(labels.price)}</th>
            <th scope="col">${escapeHtml(labels.standard)}</th>
            <th scope="col">${escapeHtml(labels.cached)}</th>
          </tr></thead>
          <tbody>${rows.map((row) => `
            <tr>
              <td><strong>${escapeHtml(row.provider)}</strong> ${escapeHtml(row.label)}</td>
              <td>$${escapeHtml(row.inputPerM)}/M${escapeHtml(labels.priceSeparator)}$${escapeHtml(row.outputPerM)}/M</td>
              <td>${fmtMoney(row.standardCost)}</td>
              <td>${fmtMoney(row.cachedEquivalentCost)}</td>
            </tr>
          `).join("")}</tbody>
        </table>
      </div>
      <div class="stats-source-note">
        ${escapeHtml(labels.localPrefix)} ${escapeHtml(managerName)} ${escapeHtml(labels.localNote)}
      </div>
    `;
  }
  ...
  function renderDetails(stats, options = {}) {
    ...
    const runtime = options.preferLiveRuntimeMetrics && stats.live ? stats.live : stats;
    const facts = runtime.facts || stats.facts || {};
    const totals = runtime.totals || stats.totals || {};
    ...
    root.innerHTML = [
      ...
      renderMiniStat(labels.kvCapacity, facts.kvCacheTokens ? `${fmtTokens(facts.kvCacheTokens)} tokens` : "-", facts.maxConcurrency ? `${labels.maxConcurrency} ${Number(facts.maxConcurrency).toFixed(2)}x` : ""),
      renderMiniStat(labels.loadWeights, facts.modelLoadSeconds ? [fmtSeconds(facts.modelLoadSeconds), facts.modelLoadMemoryGiB ? `${facts.modelLoadMemoryGiB} GiB` : ""].filter(Boolean).join(labels.separator) : "-", labels.loadStage),
      ...
      renderMiniStat(labels.source, runtime.source || stats.source || "-", `${fmtTokens(runtime.rawMetricCount || stats.rawMetricCount)} metrics`),
    ].join("");
  }
    ...
    const cards = definitions.map((definition) => {
      ...
      return `
        <article class="trend-card">
          ...
          ${renderSparkline(values, definition.color, definition.label)}
  ...
  function renderSparkline(values, color, label = "Trend") {
    const width = 240;
    const height = 64;
    ...
    return `<svg class="sparkline" viewBox="0 0 ${width} ${height}" role="img" aria-label="${defaultEscape(label)}"><title>${defaultEscape(label)}</title><polyline fill="none" stroke="${defaultEscape(color)}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" points="${coords}" /></svg>`;
  }

  function create(defaults = {}) {
    const withDefaults = (options = {}) => ({ ...defaults, ...options, labels: { ...(defaults.labels || {}), ...(options.labels || {}) } });
    return {
      statsMetric: (label, value, detail, options) => statsMetric(label, value, detail, withDefaults(options)),
      miniStat: (label, value, detail, options) => miniStat(label, value, detail, withDefaults(options)),
      shareBar: (label, value, options) => shareBar(label, value, withDefaults(options)),
      renderSparkline,
      renderCosts: (stats, options) => renderCosts(stats, withDefaults(options)),
      renderDetails: (stats, options) => renderDetails(stats, withDefaults(options)),
      renderHistoryTrends: (stats, options) => renderHistoryTrends(stats, withDefaults(options)),
    };
  }
  ...
  window.LocalAiStatsUiRenderer = api;
  window.statsUiRenderer = api;
}());
