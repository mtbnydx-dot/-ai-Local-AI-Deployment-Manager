(function () {
  ...
  function render(options = {}) {
    ...
    bindRoot(root);
    if (!items.length) {
      window.LocalAiDomUtils.setHtmlIfChanged(root, `<div class="empty">${escapeHtml(labels.empty)}</div>`);
      return;
    }
    const html = items.map((model) => renderRow(model, {
      escapeHtml,
      escapeAttr,
      fmtBytes,
      labels,
  ...
  function bindRoot(root) {
    ...
    root.addEventListener("click", (event) => {
      ...
      if (button.dataset.action === "use-model") {
        options.onUse?.({
  ...
  function create(defaults = {}) {
    return { render: (options = {}) => render({ ...defaults, ...options }) };
  }

  window.LocalAiLocalModelRenderer = { create, render };
})();
