(function () {
  function create(deps) {
      ...
      root.innerHTML = cards.map((card) => {
        ...
        return `
          <div class="trend-card">
            ...
            ${sparkline(values, { color: card.color, label: `近 2 小时${card.label}趋势` })}
    ...
    function renderExternalRecent(events) {
      ...
      if (!root) return;
      const limit = window.LocalAiDomUtils.LIST_LIMITS.standard;
      ...
      const html = rows.map((event) => {
        ...
        `;
      }).join("") + (events.length > rows.length
        ? `<div class="list-limit-note">仅显示最近 ${rows.length} / ${events.length} 条记录。</div>`
        : "");
