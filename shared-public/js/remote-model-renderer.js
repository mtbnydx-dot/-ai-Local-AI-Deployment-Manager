(function () {
  ...
  function render(options = {}) {
    ...
    renderOptions.set(root, options);
    bindRoot(root);
    const html = options.error
      ? `<div class="empty">${escapeHtml(options.errorMessage || options.error)}</div>`
      : !models.length
        ? `<div class="empty">${escapeHtml(options.emptyMessage || "No matching remote models.")}</div>`
        : models.map((model) => renderCard(model, options)).join("");
    window.LocalAiDomUtils.setHtmlIfChanged(root, html);
  ...
  function bindRoot(root) {
    ...
    root.addEventListener("click", (event) => {
      ...
      const options = renderOptions.get(root) || {};
      const action = button.dataset.action;
      if (action === "remote-readme") options.onReadme?.(button.dataset.model);
      if (action === "remote-start") options.onStart?.(button.dataset.model);
      if (action === "remote-download") {
        const allModels = Array.isArray(options.allModels) ? options.allModels : (options.models || []);
  ...
  function create(defaults = {}) {
    return {
      ...
      renderCard: (model, options = {}) => renderCard(model, { ...defaults, ...options }),
    };
  }
  ...
  window.LocalAiRemoteModelRenderer = api;
  window.remoteModelRenderer = api;
}());
