...
# Optional cursor: only chats touched after this raw updated_at value are
# exported. Passed back verbatim from a previous run's max_updated_at, so this
# never has to guess whether the column is in seconds or milliseconds.
...
"""
if SINCE is None:
    rows = con.execute(BASE_QUERY + " order by c.updated_at desc").fetchall()
else:
    rows = con.execute(
        BASE_QUERY + " where c.updated_at > ? order by c.updated_at desc", (SINCE,)
    ).fetchall()

total_chats = con.execute("select count(*) from chat").fetchone()[0]
max_updated_row = con.execute("select max(updated_at) from chat").fetchone()
max_updated_at = max_updated_row[0] if max_updated_row else None
...
markdown = [
    ...
    if SINCE is None
    else "# Open WebUI Incremental Conversation Audit Export",
    "",
    f"- Exported UTC: {export['exported_at_utc']}",
    f"- Chat count in this export: {len(rows)}",
    f"- Chats in database: {total_chats}",
    "",
]
if SINCE is not None:
    markdown.append(f"- Incremental since updated_at > {SINCE}")
    markdown.append("")
...
summary = {
    ...
    "incremental": SINCE is not None,
    "since_updated_at": SINCE,
    # Cursor for the next incremental run.
