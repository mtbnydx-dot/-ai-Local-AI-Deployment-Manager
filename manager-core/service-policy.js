...
const { listConfigHealthIssues } = require("./file-utils");
...
function globalServiceBillingClientId(settings = {}) {
  const hash = String(settings.apiKeyHash || (settings.apiKey ? hashServiceApiKey(settings.apiKey) : "")).trim().toLowerCase();
  return /^[a-f0-9]{64}$/.test(hash) ? `global-${hash.slice(0, 24)}` : "";
}
...
function normalizeServiceExposureSecret(value = {}, previous = {}) {
  // Clearing is explicit. An empty apiKey means "no new key supplied", which is
  // what every persisted record looks like -- they store the hash only. Treating
  // that empty string as "set the key to nothing" discarded the stored hash on
  // load, so a configured key stopped working after the first restart.
  if (value.clearApiKey) return { hash: "", preview: "" };
  const direct = String(value.apiKey ?? "").trim();
  if (direct) return { hash: hashServiceApiKey(direct), preview: previewServiceApiKey(direct) };
  const carriedHash = String(value.apiKeyHash || "").trim();
  if (carriedHash) return { hash: carriedHash, preview: String(value.apiKeyPreview || "") };
...
function redactServiceExposureSettings(settings = {}) {
  return {
    ...
    billingClientId: globalServiceBillingClientId(settings),
  };
}
...
function applyServiceClientUsage(client, event = {}, options = {}) {
  ...
  const explicitTotal = Number(event.usage?.total_tokens ?? event.usage?.totalTokens ?? event.totalTokens);
  const total = Number.isSafeInteger(explicitTotal) && explicitTotal >= 0 ? explicitTotal : prompt + generation;
  const cached = Number(
    event.cachedTokens
  ...
  const requestId = String(event.requestId || "").trim().slice(0, 256);
  const requestedUsageSource = String(event.usageSource || "").trim().toLowerCase();
  const usageSource = ["reported", "estimated", "missing"].includes(requestedUsageSource)
    ? requestedUsageSource
    : ((prompt || generation || total) ? "reported" : "missing");
  const requestedTerminalState = String(event.terminalState || "").trim().toLowerCase();
  const terminalState = ["completed", "failed", "aborted", "timed_out"].includes(requestedTerminalState)
  ...
  return {
    ...
    event: {
      ...
      ...(event.stream !== undefined ? { stream: event.stream === true } : {}),
      ...(event.terminalState !== undefined ? { terminalState } : {}),
      ...(Number.isSafeInteger(cached) && cached >= 0 ? { cachedTokens: cached } : {}),
    },
  };
}
...
function buildServiceExposurePayloadSnapshot(settings = {}, context = {}, options = {}) {
  ...
  return {
    ...
    configHealth: {
      ok: !listConfigHealthIssues().length,
      issues: listConfigHealthIssues(),
    },
    actual: {
      manager: {
...
function buildServiceExposureChecks(settings = {}, context = {}, options = {}) {
  ...
  // A settings file that failed to parse used to fall back to defaults, which
  // silently turned API key enforcement off. It now fails closed, and the
  // reason has to be visible here rather than only in the process log.
  const configIssues = Array.isArray(context.configIssues) ? context.configIssues : listConfigHealthIssues();
  if (configIssues.length) {
    checks.push(serviceCheck(
      "fail",
      "配置文件完整性",
      `${configIssues.length} 个配置文件无法解析，服务网关已 fail-closed 拒绝请求：${configIssues.map((item) => item.file).join("、")}`,
    ));
  }
