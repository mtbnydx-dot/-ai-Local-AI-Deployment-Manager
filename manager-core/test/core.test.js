...
test("gateway helpers extract common auth fields", () => {
  ...
  assert.equal(core.resolveOpenAiGatewayModel("/models/qwen-root", {
    servedModels: [{ id: "Qwen/Qwen3-27B", root: "/models/qwen-root" }],
  }), "Qwen/Qwen3-27B");
});
...
test("stats helpers aggregate model totals and calculate API value", () => {
  const totals = core.aggregateStats([
    {
      ...
      speed: { recentPromptTokensPerSecond: 10, recentOutputTokensPerSecond: 4, recentRequestsPerMinute: 2, outputSource: "sglang_recent_gauge", outputObservedAt: 1000 },
      latency: { avgE2eSeconds: 2, avgTtftSeconds: 0.5, avgTimePerOutputTokenSeconds: 0.1 },
      context: { activeTokens: 1000, capacityTokens: 4000 },
      cache: { prefixQueries: 10, prefixHits: 8, alignmentBlockSize: 32, alignmentRequired: true },
      speculative: { draftCycles: 10, draftTokens: 30, acceptedTokens: 24 },
    },
    {
      tokens: { prompt: 50, generation: 10, cachedPrompt: 0 },
      requests: { total: 1, error: 0, aborted: 0 },
      speed: { recentPromptTokensPerSecond: 5, recentOutputTokensPerSecond: 1, recentRequestsPerMinute: 1, outputSource: "sglang_recent_gauge", outputObservedAt: 900 },
      latency: { avgE2eSeconds: 6, avgTtftSeconds: 1, avgTimePerOutputTokenSeconds: 0.2 },
      context: { activeTokens: 500, capacityTokens: 1000 },
      cache: { prefixQueries: 5, prefixHits: 2 },
      speculative: { draftCycles: 5, draftTokens: 10, acceptedTokens: 8 },
    },
  ], 10);
  ...
  assert.equal(totals.speed.outputSource, "sglang_recent_gauge");
  assert.equal(totals.speed.outputObservedAt, 1000);
  assert.equal(totals.context.kvUsagePercent, 0.3);
  assert.equal(totals.cache.prefixHitRate, 10 / 15);
  assert.equal(totals.cache.alignmentBlockSize, 32);
  assert.equal(totals.speculative.acceptanceRate, 32 / 40);
  assert.equal(totals.speculative.acceptedTokensPerCycle, 32 / 15);
...
test("service gateway middleware enforces auth, CORS, limits, and emits metadata logs", async () => {
  ...
  const globalSecret = "sk-global-billing-test";
  const globalSettings = {
    ...
    maxConcurrentRequests: 2,
    requestTimeoutSeconds: 45,
    apiKeyHash: core.hashServiceApiKey(globalSecret),
  };
  const globalMiddleware = core.createServiceGatewayMiddleware({
    gatewayName: "global-key-manager",
    supportedKinds: ["openai"],
    getServiceExposureSettings: async () => globalSettings,
    getServiceClientsLedger: async () => ({ clients: [] }),
    resolveServiceClientForApiKey: async () => null,
    rateBuckets: new Map(),
    concurrencyBuckets: new Map(),
  ...
  const globalReq = {
    method: "POST",
    originalUrl: "/serve/v1/chat/completions",
    headers: { authorization: `Bearer ${globalSecret}` },
    socket: { remoteAddress: "192.168.1.50" },
    body: { model: "requested" },
  };
  const globalResponse = makeResponse();
  await globalMiddleware(globalReq, globalResponse.res, () => {});
  assert.equal(globalReq.serviceGateway.clientId, core.globalServiceBillingClientId(globalSettings));
  assert.match(globalReq.serviceGateway.clientId, /^global-[a-f0-9]{24}$/);
  assert.equal(core.redactServiceExposureSettings(globalSettings).billingClientId, globalReq.serviceGateway.clientId);
  globalResponse.res.trigger("finish");
...
test("docker image pull retries transient registry failures with backoff", async () => {
  ...
  assert.deepEqual(commands, [
    ["pull", "--platform", "linux/amd64", "repo/image:v1"],
    ["pull", "--platform", "linux/amd64", "repo/image:v1"],
    ["pull", "--platform", "linux/amd64", "repo/image:v1"],
  ]);
});

test("docker runtime supports a WSL command prefix and detached daemon keepalive", async () => {
  const calls = [];
  const spawned = [];
  const responses = [
    { stdout: "Docker version 29.7.2\n" },
    { error: new Error("not running"), stderr: "daemon unavailable" },
    { error: new Error("not running"), stderr: "daemon unavailable" },
    { stdout: "29.7.2\n" },
  ];
  const runtime = core.createDockerRuntime({
    dockerExe: "wsl.exe",
    dockerArgsPrefix: ["-d", "Qwen38-SGLang", "--exec", "docker"],
    daemonStartFile: "wsl.exe",
    daemonStartArgs: ["-d", "Qwen38-SGLang", "--exec", "sleep", "infinity"],
    runtimeName: "WSL Qwen38-SGLang",
    delay: async () => {},
    spawnCommand: (file, args, options) => {
      spawned.push([file, args, options.detached, options.windowsHide]);
      return { unref: () => spawned.push(["unref"]) };
    },
    execFileCommand: (file, args, _options, callback) => {
  ...
  const started = await runtime.ensureDockerDaemonRunning(5000);
  assert.equal(started.ok, true);
  assert.equal(started.runtimeName, "WSL Qwen38-SGLang");
  assert.deepEqual(calls[0], ["wsl.exe", ["-d", "Qwen38-SGLang", "--exec", "docker", "--version"]]);
  assert.deepEqual(calls[1], ["wsl.exe", ["-d", "Qwen38-SGLang", "--exec", "docker", "info", "--format", "{{.ServerVersion}}"]]);
  assert.deepEqual(spawned[0], ["wsl.exe", ["-d", "Qwen38-SGLang", "--exec", "sleep", "infinity"], true, true]);
});
...
test("service usage store persists client rows and usage events without prompt content", () => {
  ...
  const db = {
    ...
    prepare(sql) {
      if (/PRAGMA table_info\(service_usage_events\)/i.test(sql)) {
        return {
          all: () => [
            "event_id", "at", "manager", "client_id", "model", "status", "ok",
            "prompt_tokens", "generation_tokens", "total_tokens", "request_id",
            "usage_source", "stream", "terminal_state", "cached_tokens",
          ].map((name) => ({ name })),
        };
      }
  ...
  core.persistServiceUsageEventToDb(db, {
    ...
    apiKey: "test-key-must-not-be-persisted",
  }, {
    eventId: "evt-1",
    now: "2026-06-15T00:01:00.000Z",
    managerId: "vllm-manager",
  ...
  assert.match(statements[1].sql, /INSERT OR IGNORE INTO service_usage_events/);
  assert.deepEqual(statements[1].runs[0], [
    "evt-1", "2026-06-15T00:01:00.000Z", "vllm-manager", "client-1", "local", 200, 1, 5, 7, 12,
    "request-1", "reported", 1, "completed", 3,
  ]);
  assert.equal(statements[1].runs[0].some((value) => String(value).includes("must-not-be-persisted")), false);
...
test("job route registrar handles download and engine-specific cancel actions", async () => {
  ...
  const downloadJob = { id: "download-1", type: "download", status: "running", logs: ["a", "b", "c"] };
  ...
  let response = makeResponse();
  await routes.get("GET /api/jobs")({ query: {} }, response.res);
  assert.deepEqual(response.state.json[0].map((job) => job.id), ["serve-1", "download-1"]);
  response = makeResponse();
  await routes.get("GET /api/jobs")({ query: { fields: "summary" } }, response.res);
  assert.equal(response.state.json[0][1].logCount, 3);
  response = makeResponse();
  await routes.get("GET /api/jobs/:id/logs")({ params: { id: "download-1" } }, response.res);
  assert.deepEqual(response.state.json[0].logs, ["a", "b", "c"]);
  ...
  assert.deepEqual(calls, ["read", "read", "read", "read", "cancel:download-1", "pause:download-1", "resume:download-1"]);
...
test("benchmark runner normalizes requests and records samples", async () => {
  ...
  assert.deepEqual(runner.normalizeBenchmarkRequest({ requests: "invalid", concurrency: 99, warmupRequests: -2, maxTokens: 19.9 }), {
    port: 8080,
    model: "",
    requests: 3,
    concurrency: 3,
  ...
  const result = await runner.runBenchmarkJob(job, { requests: 2, concurrency: 2, maxTokens: 32 });
  ...
  assert.equal(result.totalOutputTokens, 10);
  assert.equal(Number.isFinite(result.wallTokensPerSecond), true);
  assert.equal(job.status, "success");
  assert.equal(job.meta.benchmark.model, "model-a");
  assert.match(job.logs[0], /Run 1:/);
});

test("benchmark runner recognizes vLLM 0.28 reasoning stream tokens", async () => {
  const encoder = new TextEncoder();
  const runner = core.createBenchmarkRunner({
    defaultPort: 8000,
    defaultPrompt: "think",
    runtimeLabel: "vLLM",
    requestDetail: "reasoning stream",
    getRunningModelSummary: async () => ({
      container: { running: true },
      endpoint: { port: 8000 },
      models: [{ id: "model-a" }],
    ...
    fetchImpl: async () => ({
      ...
      body: new ReadableStream({
        start(controller) {
          ...
          controller.enqueue(encoder.encode('data: {"usage":{"prompt_tokens":3,"completion_tokens":2},"choices":[]}\n\n'));
  ...
  const result = await runner.runBenchmarkJob({ logs: [], meta: {} }, {
    requests: 1,
    maxTokens: 16,
    stream: true,
  });
  ...
  assert.equal(result.samples[0].outputTokens, 2);
  assert.equal(result.samples[0].preview, "thinking");
});
...
test("model route registrar wires shared and optional model download endpoints", async () => {
  ...
  core.registerModelRoutes(app, {
    ...
    getHfAuthStatus: async () => ({ connected: true, username: "alice" }),
    launchHfLogin: async () => ({ launched: true }),
    checkHfAccess: async (query) => ({ model: query.model, granted: true }),
    isLocalRequest: (req) => Boolean(req.local),
  ...
  startDownloadImpl = async () => {
    ...
    error.status = 400;
    error.code = "hf_login_required";
    error.authorizationUrl = "https://huggingface.co/owner/model";
    throw error;
  };
  response = makeResponse();
  await routes.get("POST /api/download")({ body: { model: "owner/model" } }, response.res);
  assert.equal(response.state.statusCode, 400);
  assert.deepEqual(response.state.json[0], {
    error: "bad source",
    code: "hf_login_required",
    authorizationUrl: "https://huggingface.co/owner/model",
  });
  ...
  response = makeResponse();
  await routes.get("GET /api/huggingface/auth/status")({ local: true }, response.res);
  assert.deepEqual(response.state.json[0], { connected: true, username: "alice" });

  response = makeResponse();
  await routes.get("POST /api/huggingface/auth/login")({ local: true }, response.res);
  assert.deepEqual(response.state.json[0], { launched: true });

  response = makeResponse();
  await routes.get("GET /api/huggingface/access")({ local: true, query: { model: "owner/model" } }, response.res);
  assert.deepEqual(response.state.json[0], { model: "owner/model", granted: true });

  response = makeResponse();
  await routes.get("POST /api/huggingface/auth/login")({ local: false }, response.res);
  assert.equal(response.state.statusCode, 403);
...
test("download helpers normalize precision aliases and GGUF include patterns", () => {
  ...
  assert.deepEqual(core.buildDownloadIncludePatterns("FP8", "orcarouter/Qwen3.8-27B-Uncensored-FP8"), []);
  assert.equal(core.modelReferenceDeclaresPrecision("owner/model-FP8", "FP8"), true);
  assert.equal(core.modelReferenceDeclaresPrecision("fp8-owner/model", "FP8"), false);
  ...
  assert.deepEqual(core.filterDownloadSiblings(siblings, "Q6_K"), []);
  assert.ok(core.buildDownloadIncludePatterns("FP8").some((pattern) => pattern.includes("safetensors")));
  assert.throws(
    () => core.assertDownloadDiskSpace(100 * 1024 ** 3, 20 * 1024 ** 3, (value) => String(value)),
    (error) => error.status === 400 && error.code === "download_disk_full",
  );
  ...
  const dedicatedFp8Siblings = [
    { rfilename: "model-00001-of-00002.safetensors", size: 10 },
    { rfilename: "model-00002-of-00002.safetensors", size: 20 },
    { rfilename: "config.json", size: 5 },
  ];
  assert.deepEqual(core.selectDownloadSiblings(dedicatedFp8Siblings, "FP8", "owner/model-FP8"), {
    siblings: dedicatedFp8Siblings,
    includePatterns: [],
    filtered: false,
    matched: 3,
    total: 3,
  ...
  const fp8Spec = builder.buildDownloadSpecFromJob({
    meta: { model: "orcarouter/Qwen3.8-27B-Uncensored-FP8", precision: "FP8" },
  });
  assert.deepEqual(fp8Spec.args, [
    "download",
    "orcarouter/Qwen3.8-27B-Uncensored-FP8",
    "--local-dir",
    "D:/models/orcarouter__Qwen3.8-27B-Uncensored-FP8",
  ]);
...
test("job helpers normalize persistence, logs, issues, and byte labels", () => {
  ...
  assert.deepEqual(core.extractLogIssues("Traceback (most recent call last):\nOSError: libavutil.so.60: cannot open shared object file: No such file or directory\nOSError: Could not load this library: /opt/torchcodec/libtorchcodec_core8.so\n[end of libtorchcodec loading traceback]."), []);
});
...
test("stats ledger store persists deltas, Claude usage, and runtime facts", async () => {
  ...
  await store.updateStatsLedger({
    ...
    facts: { kvCacheTokens: 373883, maxContextTokens: 262144 },
    models: [{
      name: "org/model-a",
      engine: "sglang",
      runtimeVersion: "0.5.17",
      root: "D:/AI/models/model-a",
      tokens: { prompt: 10, generation: 5, cachedPrompt: 2 },
      requests: { total: 1, success: 1, error: 0, aborted: 0 },
      context: { activeTokens: 0, capacityTokens: 373883, kvUsagePercent: 0, maxModelLen: 262144, concurrencyAtMaxLen: 1.43 },
      speculative: { enabled: true, mode: "dspark", acceptanceRate: 0.8 },
    }],
  }, "collect");
  const engineLedger = await store.loadStatsLedger();
  assert.ok(engineLedger.runtimes["sglang:200:org/model-a"]);
  assert.equal(engineLedger.runtimeFacts["sglang:org/model-a"].engine, "sglang");
  const sglangFacts = await store.getPersistedRuntimeFacts(["model-a"], { engine: "sglang" });
  assert.equal(sglangFacts.kvCacheTokens, 373883);
  const vllmFacts = await store.getPersistedRuntimeFacts(["model-a"], { engine: "vllm" });
  assert.equal(vllmFacts.maxConcurrency, 2);
...
test("download controller rejects a second active job for the same localDir and skips deleting a busy directory", async () => {
  const jobs = new Map();
  const removed = [];
  const createJob = (type, title, meta = {}) => {
    const job = core.createJobRecord(type, title, meta, { id: `job-${jobs.size + 1}` });
  ...
  const controller = core.createDownloadJobController({
    jobs,
    downloadSpecs: new Map(),
    createJob,
    spawnJobProcess: (job) => { job.status = "running"; },
    buildDownloadSpecFromJob: (job) => ({ command: "hf", args: ["download"], options: { meta: job.meta } }),
    appendLog: (job, data) => { job.logs = [...(job.logs || []), String(data)]; },
    stopProgressTracker: () => {},
    resolvePartialPath: (value) => value,
    removePartialPath: async (target) => { removed.push(target); },
  });
  const first = controller.enqueueOrStartDownload("hf", ["download", "same"], { title: "A", meta: { model: "same", localDir: "D:/models/same" } });
  assert.equal(first.status, "running");
  assert.throws(
    () => controller.enqueueOrStartDownload("hf", ["download", "same"], { title: "B", meta: { model: "same", localDir: "D:/models/same" } }),
    (error) => error.status === 409 && error.existingJobId === first.id,
  );
  const other = createJob("download", "busy", { localDir: "D:/models/same" });
  other.status = "running";
  const cancelled = createJob("download", "cancel-me", { localDir: "D:/models/same" });
  cancelled.status = "paused";
  await controller.deletePartialDownload(cancelled);
  assert.deepEqual(removed, []);
  assert.ok((cancelled.logs || []).some((line) => /跳过删除/.test(line)));
});

test("failed downloads from before process start are not auto-retried", async () => {
  const jobs = new Map();
  const scheduled = [];
  const stale = core.createJobRecord("download", "old", { model: "old", failedAt: 1, retryCount: 0, maxRetries: 2 }, { id: "stale" });
  stale.status = "failed";
  jobs.set(stale.id, stale);
  const fresh = core.createJobRecord("download", "new", { model: "new", failedAt: Date.now() + 1000, retryCount: 0, maxRetries: 2 }, { id: "fresh" });
  fresh.status = "failed";
  ...
  const controller = core.createDownloadJobController({
    ...
    createJob: () => { throw new Error("unused"); },
    spawnJobProcess: () => {},
    buildDownloadSpecFromJob: (job) => ({ command: "hf", args: ["download"], options: { meta: job.meta } }),
    appendLog: (job, data) => { job.logs = [...(job.logs || []), String(data)]; },
    stopProgressTracker: () => {},
    setRetryTimeout: () => {
      const job = Array.from(jobs.values()).find((item) => item.meta?.retryScheduledAt && !scheduled.includes(item.id));
      if (job) scheduled.push(job.id);
      return 1;
    },
  });
  controller.drainDownloadQueue();
  assert.equal(scheduled.includes("stale"), false);
  assert.equal(scheduled.includes("fresh"), true);
  assert.ok((stale.logs || []).some((line) => /不再自动续传/.test(line)));
});

test("shutdown helper terminates a live download process and marks it interrupted", async () => {
  const { spawn } = require("node:child_process");
  const child = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)"], { windowsHide: true, stdio: "ignore" });
  const jobs = new Map();
  const job = core.createJobRecord("download", "live", { model: "x" }, { id: "live-dl" });
  job.status = "running";
  job.pid = child.pid;
  jobs.set(job.id, job);
...
test("pending billing queue replays authorize then settle", async () => {
  const authorized = [];
  const settled = [];
  const queue = core.createBillingPendingQueue({
  ...
  queue.enqueue({
    authorizeInput: { requestId: "req-1", clientId: "c1" },
    settleInput: { requestId: "req-1", inputTokens: 3 },
  });
  const result = await queue.flush();
  assert.equal(result.flushed, 1);
  assert.equal(authorized.length, 1);
  assert.equal(settled[0].inputTokens, 3);
});
