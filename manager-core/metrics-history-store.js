...
function buildMetricsHistorySample(summary = {}, options = {}) {
  ...
  return {
    ...
    speculative: {
      ...
      mode: String(speculative.mode || ""),
      draftCycles: finiteNumber(speculative.draftCycles),
      ...
      acceptedTokensPerCycle: finiteNumber(speculative.acceptedTokensPerCycle),
      draftTokensPerCycle: finiteNumber(speculative.draftTokensPerCycle),
      meanAcceptLength: finiteNumber(speculative.meanAcceptLength),
    },
  };
}
...
function createMetricsHistoryStore(options = {}) {
  ...
  async function getMetricsHistory(query = {}) {
    ...
    const engine = String(query.engine || "").trim().toLowerCase();
    const samples = ledger.samples.filter((sample) => {
      if (Date.parse(sample.at) < fromMs) return false;
      if (model && String(sample.model || "").toLowerCase() !== model) return false;
      return !engine || String(sample.engine || "").toLowerCase() === engine;
    });
