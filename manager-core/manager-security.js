...
const { getServiceGatewayKind, resolveGatewayRequestOrigin } = require("./gateway-utils");
...
// A signed gateway hop carries the real client address; if that address is not
// itself loopback the request came from the LAN and must not inherit local trust.
function isRemoteProxiedRequest(origin) {
...
function createManagerSecurityGuard(options = {}) {
  ...
  function managerSecurityGuard(req, res, next) {
    ...
    // A loopback hop that claims to come from service-entry but cannot prove it
    // never counts as local, whatever the caller-supplied predicate says.
    const requestOrigin = resolveGatewayRequestOrigin(req);
    const isLocal = requestOrigin.viaGateway && !requestOrigin.trusted
      ? false
      : localRequest(req) && !isRemoteProxiedRequest(requestOrigin);
