...
function estimateLlamaMemoryPlan({
  ...
  const explicitKvCacheGb = Number(kvCacheGb);
  const hasExplicitKvCacheGb = kvCacheGb !== null && kvCacheGb !== undefined && String(kvCacheGb).trim() !== "";
  const totalKvGb = hasExplicitKvCacheGb && Number.isFinite(explicitKvCacheGb) && explicitKvCacheGb >= 0
    ? explicitKvCacheGb
    : estimateKvCacheGiB({ contextTokens, ...modelArch, kvBytes });
